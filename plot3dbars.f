C
C******************************************************************************
C Realiza un dibujo tridimensional de la region alrededor del pixel XC,YC
...
C MM3D=.TRUE. limites fijos (se toman BG3D y FG3D como input)
        SUBROUTINE PLOT3DBARS(NC1,NC2,NS1,NS2,
     +   IMODE,MM3D,BG3D,FG3D)
        IMPLICIT NONE
...
C
        ...
        DX=XMAX-XMIN
C------------------------------------------------------------------------------
C obtenemos la region de dibujo actual
        CALL PGQVP(0,XV1,XV2,YV1,YV2)
        CALL PGQFS(OLD_FS)
        CALL PGSFS(1)
C definimos region de dibujo para imagen 3D
