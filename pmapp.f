...
C------------------------------------------------------------------------------
        CALL PGBBUF
C------------------------------------------------------------------------------
C dibujamos extrapolacion hacia abajo
        IF(SYMINEXTG.GT.0)THEN
          ...
          DO WHILE(NY0.GE.NY1)
            Y0=REAL(NY0)
            DO I=1,NXYMAX
              X0=REAL(SXMINGRID-SXMINEXTG)+REAL(I-1)/REAL(NXYMAX-1)*
     +         REAL(SXMAXGRID+SXMAXEXTG-SXMINGRID+SXMINEXTG)
          ...
          DO WHILE(NX0.LE.NX1)
            X0=REAL(NX0)
            DO I=1,NXYMAX
              Y0=REAL(SYMINGRID-SYMINEXTG)+REAL(I-1)/REAL(NXYMAX-1)*
     +         REAL(SYMINEXTG)
              CALL FMAP(NMAP,AIJ_,BIJ_,X0,Y0,XP(I),YP(I))
            END DO
            CALL PGSCI(5)
          ...
          END DO
        END IF
C dibujamos extrapolacion hacia arriba
     ...
     +         REAL(SXMAXGRID+SXMAXEXTG-SXMINGRID+SXMINEXTG)
          ...
          DO WHILE(NX0.LE.NX1)
            X0=REAL(NX0)
            DO I=1,NXYMAX
              Y0=REAL(SYMAXGRID)+REAL(I-1)/REAL(NXYMAX-1)*
     +         REAL(SYMAXEXTG)
              CALL FMAP(NMAP,AIJ_,BIJ_,X0,Y0,XP(I),YP(I))
            END DO
            CALL PGSCI(5)
          ...
          END DO
        END IF
C dibujamos extrapolacion hacia la izquierda
     ...
     +         REAL(SXMINEXTG)
          ...
          END DO
        END IF
C dibujamos extrapolacion hacia la derecha
     ...
     +         REAL(SYMAXGRID+SYMAXEXTG-SYMINGRID+SYMINEXTG)
          ...
          DO WHILE(NY0.LE.NY1)
            Y0=REAL(NY0)
            DO I=1,NXYMAX
              X0=REAL(SXMAXGRID)+REAL(I-1)/REAL(NXYMAX-1)*
     +         REAL(SXMAXEXTG)
              CALL FMAP(NMAP,AIJ_,BIJ_,X0,Y0,XP(I),YP(I))
            END DO
            CALL PGSCI(5)
        ...
        END IF
C------------------------------------------------------------------------------
C dibujamos una malla equidistante de paso NGRID (usamos variables enteras
C para evitar errores de redondeo al avanzar en el grid)
        NY0=SYMINGRID
        NY1=SYMAXGRID
        DO WHILE(NY0.LE.NY1)
          Y0=REAL(NY0)
          DO I=1,NXYMAX
            X0=REAL(SXMINGRID)+REAL(I-1)/REAL(NXYMAX-1)*
     +       REAL(SXMAXGRID-SXMINGRID)
            CALL FMAP(NMAP,AIJ_,BIJ_,X0,Y0,XP(I),YP(I))
          END DO
          CALL PGSCI(3)
          CALL PGLINE(NXYMAX,XP,YP)
        ...
        DO WHILE(NX0.LE.NX1)
          X0=REAL(NX0)
          DO I=1,NXYMAX
            Y0=REAL(SYMINGRID)+REAL(I-1)/REAL(NXYMAX-1)*
     +       REAL(SYMAXGRID-SYMINGRID)
            CALL FMAP(NMAP,AIJ_,BIJ_,X0,Y0,XP(I),YP(I))
          END DO
          CALL PGSCI(3)
          CALL PGLINE(NXYMAX,XP,YP)
        ...
        END DO
C------------------------------------------------------------------------------
C dibujamos perimetro del grid (incluyendo las extrapolaciones)
C borde inferior
