C Refina la posicion de una linea BA del boundary.
C Nota: el polinomio de entrada en la subrutina es modificados a la salida.
C X0 es un valor de X apropiado para buscar las intersecciones con los
C espectros del borde.
...
C------------------------------------------------------------------------------
        LDEBUGLOCAL=.TRUE.
C intersecciones del polinomio temporal con los espectros del borde
        CALL INTERSEC(NDEG,COEFF,NDEGBL(1),COEFFBL(1,1),
     +   X0,XMIN,YMIN)
        CALL INTERSEC(NDEG,COEFF,NDEGBL(NLINBL),COEFFBL(1,NLINBL),
     +   X0,XMAX,YMAX)
        IF(LDEBUGLOCAL)THEN
          CALL PGSCI(5)
...
C------------------------------------------------------------------------------
        ...
        DO I=I1,I2
          ...
          END DO
          CALL GAUSCFIT(NWIDTH,XF,YF,X0_,SIGMA,AMP,Y0_,
     +     EEX0_,EESIGMA,EEAMP,EEY0_,1.E-6)
ccc          CALL POLFIT(XF,YF,YF,NWIDTH,3,0,COEFFPAR,CHISQR)
ccc          XPEAKS(I-I1+1)=-COEFFPAR(2)/2./COEFFPAR(3)
          XPEAKS(I-I1+1)=X0_
          YPEAKS(I-I1+1)=REAL(I)
        END DO
        NPEAKS=I2-I1+1
        CALL POLFITSIG(NPEAKS,YPEAKS,XPEAKS,TSIGMA,NDEG,COEFF,LFIT)
C en lugar de dibujar el ajuste directamente es mejor mostrar los residuos
        DO J=1,NPEAKS
        ...
        IF(LDEBUGLOCAL)THEN
          ...
          YMIN_=-1.
          YMAX_=+1.
          CALL SUBPLOT(NPEAKS,1,NPEAKS,YPEAKS,RPEAKS,YPEAKS,RPEAKS,
     +     .TRUE.,.FALSE.,.FALSE.,.FALSE.,
     +     'spatial direction','residuals in wavelength direction',' ',
     +     3,1,1.5)
          J=0
          DO I=1,NPEAKS
            IF(.NOT.LFIT(I))THEN
            ...
            END IF
          END DO
          IF(J.GT.0) CALL SUBPLOTBIS(J,1,J,YF,XF,YF,XF,
     +     .FALSE.,.FALSE.,2,5,1.0)
          DO I=1,NXYMAX
          ...
          CALL PGSCI(1)
          CALL SUBPLOTBIS(NXYMAX,1,NXYMAX,YF,XF,YF,XF,
     +     .FALSE.,.FALSE.,4,101,1.0)
