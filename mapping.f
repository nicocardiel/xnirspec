...
C------------------------------------------------------------------------------
        ELSEIF(COPC.EQ.'l')THEN
          ...
          DO WHILE(.NOT.LOGFILE)
            FILENAME=READC('Mapping file name (none=EXIT)',
     +       '*.mapp','@')
...
C OJO: el numero de pixels no es I2-I1+1, sino que es I2-I1; por eso los bucles
C van desde I1 hasta I2-1. I1,I2,J1,J2 se refieren a los bordes de los pixels,
C no al numero de pixel. Hay un offset de 0.5 entre el mapeado realizado con
C los coeficientes AIJ,BIJ,AIJ_,BIJ_ y el numero de pixel en la imagen
C original.
            NDELTAX=J2-J1
            NDELTAY=I2-I1
...
C------------------------------------------------------------------------------
        ELSEIF(COPC.EQ.'t')THEN
          ...
          DO WHILE(CH.NE.'X')
            ...
            ELSE
              ...
              DO I=1,NXYMAX
                VV=REAL(SYMINGRID-SYMINEXTG)+REAL(I-1)/REAL(NXYMAX-1)*
     +           REAL(SYMAXGRID+SYMAXEXTG-SYMINGRID+SYMINEXTG)
              ...
              ALPHA0_=AIJ_(1)+AIJ_(4)*(UU*FSCALE)+
     +         AIJ_(6)*(UU*FSCALE)*(UU*FSCALE)
              ALPHA1_=AIJ_(2)+AIJ_(5)*(UU*FSCALE)
              ALPHA2_=AIJ_(3)
              NEXTINFO=0
              DO I=I1,I2
                YY=REAL(I)
                ALPHA0=BIJ(1)+BIJ(2)*(YY*FSCALE)+
     +           BIJ(3)*(YY*FSCALE)*(YY*FSCALE)
                ...
                BETA(3)=ALPHA1_*ALPHA2+2.*ALPHA2_*ALPHA0*ALPHA2+
     +           ALPHA2_*ALPHA1*ALPHA1
                ...
                END DO
                CALL GAUSCFIT(NWIDTH,XF,YF,X0,SIGMA,AMP,Y0,
     +           EEX0,EESIGMA,EEAMP,EEY0,1.E-6)
                ...
                IF(CMORE.EQ.'y')THEN
                  XMIN_=REAL(J1)-0.6
                  XMAX_=REAL(J2)+0.6
                  CALL SUBPLOT(NWIDTH,1,NWIDTH,XF,YF,XF,YF,
     +             .FALSE.,.TRUE.,.FALSE.,.FALSE.,
     +             'y-axis','signal',' ',3,201,1.0)
                  DO II=1,NXYMAX
                    XP_(II)=REAL(J1)+
     +               REAL(II-1)/REAL(NXYMAX-1)*REAL(J2-J1)
                    ...
                    END IF
                  END DO
                  CALL SUBPLOTBIS(NXYMAX,1,NXYMAX,XP_,YP_,XP_,YP_,
     +              .FALSE.,.FALSE.,5,101,1.0)
              ...
              CALL SUBPLOT(NP,1,NP,YP,RP,YP,RP,
     +         .TRUE.,.FALSE.,.FALSE.,.FALSE.,
     +         'spatial direction','offset residuals',' ',
     +         3,1,1.5)
            END IF
          END DO
          RETURN
...
C------------------------------------------------------------------------------
C------------------------------------------------------------------------------
C Aqui empieza la opcion 'c': calcular el mapping a partir del boundary
C------------------------------------------------------------------------------
C------------------------------------------------------------------------------
C dibujamos el boundary con todas las intersecciones
        CALL PBOUND
C..............................................................................
C origen de medida de longitudes de arco
        NREFBL=READILIM('NREFBL','1',1,NLINBL)
        NREFBA=READILIM('NREFBA','1',1,NLINBA)
        CALL INTERSEC(NDEGBA(NREFBA),COEFFBA(1,NREFBA),
...
c
     ...
     +       0.5*(XMINBA(L)+XMAXBA(L)),X(K),Y(K))
            CALL PGPOINT(1,X(K),Y(K),17)
c
            CALL INTERSEC(NDEGBA(L),COEFFBA(1,L),
     +       NDEGBL(NREFBL),COEFFBL(1,NREFBL),
     +       0.5*(XMINBA(L)+XMAXBA(L)),X1,Y1)
            U(K)=ARCLENGTH(NDEGBL(NREFBL),COEFFBL(1,NREFBL),
     +       X0,X1,100)
c
            CALL INTERSEC(NDEGBA(NREFBA),COEFFBA(1,NREFBA),
     +       NDEGBL(LL),COEFFBL(1,LL),
     +       0.5*(XMINBA(NREFBA)+XMAXBA(NREFBA)),X1,Y1)
            V(K)=ARCLENGTH(NDEGBA(NREFBA),COEFFBA(1,NREFBA),
     +       Y0,Y1,100)
          END DO
        END DO
        NFIT=K
...
C mostramos coeficientes bij
        ...
        END IF
C..............................................................................
C matriz del tercer sistema de ecuaciones a resolver
        II=0
        DO L=0,N
          DO M=0,N-L
            II=II+1
...
C resolvemos el sistema de ecuaciones
        ...
        WRITE(*,101)' OK!'
C..............................................................................
C cuarto sistema de ecuaciones a resolver (notar que la matriz
C del sistema de ecuaciones es la misma)
        II=0
        DO L=0,N
          DO M=0,N-L
...
C mostramos coeficientes bij
        ...
        END IF
C..............................................................................
C dibujamos el grid con el nuevo ajuste
        CALL PGBBUF
...
C limites del grid
        ...
        END IF
C------------------------------------------------------------------------------
C si se desea, podemos extrapolar el grid
        CEXT(1:1)=READC('Are you extrapolating the grid (y/n)','n','yn')
        IF(CEXT.EQ.'y')THEN
          SXMINEXTG=READILIM('No. of pixels (X<Xmin)','0',0,
     +     NXMAX-NDELTAX)
          SXMAXEXTG=READILIM('No. of pixels (X>Xmax)','0',0,
     +     NXMAX-NDELTAX-SXMINEXTG)
          SYMINEXTG=READILIM('No. of pixels (Y<Ymin)','0',0,
     +     NYMAX-NDELTAY)
          SYMAXEXTG=READILIM('No. of pixels (Y>Ymax)','0',0,
     +     NYMAX-NDELTAY-SYMINEXTG)
        ELSE
          SXMINEXTG=0
          SXMAXEXTG=0
          SYMINEXTG=0
