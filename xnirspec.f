C******************************************************************************
C Version 26-March-2002                (c) Nicolas Cardiel: cardiel@ucolick.org
C                                                         ncl@astrax.fis.ucm.es
C******************************************************************************
C g77 -o xnirspec *.o -L$PGPLOT_DIR -L$FITSIO_DIR -L/usr/X11/lib \
...
C functions
        ...
        REAL FPOLY
        CHARACTER*255 READC
C variables que definen el boundary
C NOTA: Por razones historicas, los polinomios que definen el boundary se 
C denominan BL y BA por Boundary Lines, y Boundary Arc lines. 
C Cuando se decidio rotar las imagenes para tener dispersion en el 
...
C------------------------------------------------------------------------------
C Open graphic display and establish button and plot settings
        TTER='/XSERVE'
...
C change uppercase by lowercase when appropriate
          IF(CH.EQ.'Z') CH='z'
          IF(CH.EQ.'W') CH='w'
C look for accelerators
...
c..............................................................................
     ...
     +             ', Y='//CIYC(1:L2)//', signal='//CSIGNAL(1:L3)
                  IF(LMAPPING)THEN
                    ...
                    WRITE(CIYC,*) V
                    CALL RMBLANK(CIYC,CIYC,L2)
                    WRITE(*,101) '=> Sx='//CIXC(1:L1)//
     +               ', Sy='//CIYC(1:L2)
                    IF(LPWAVE)THEN
                      WRITE(*,100) '=> Wavelength: '
                      WRITE(*,*) FPOLY(NDEGWV,CWV,
     +                 U+0.5+REAL(SXMINEXTG))
                    END IF
                  END IF
                  IF(LSTACK)THEN
...
C------------------------------------------------------------------------------
          ELSEIF(NB.EQ.1)THEN
            ...
            DO WHILE(.NOT.LOGFILE)
              INFILE_=READC(
     +         'Input FITS file name (none=EXIT)',
     +         '*.*fit* FITS*/*.fits *.b9','@')
...
C..............................................................................
            IF(.NOT.LOGFILERR)THEN                 !the file name is not 'none'
              ...
              DO WHILE(LASK)
                ...
                END IF
                LASK=(IROTATE.NE.0).AND.(IABS(IROTATE).NE.90).AND.
     +           (IABS(IROTATE).NE.180).AND.(IABS(IROTATE).NE.270)
                IF(LASK)THEN
                  WRITE(*,101) '***ERROR***'
...
ccc                 J1=256
                  ...
                  IF(CINERR.EQ.'g')THEN
                    DO I=1,NAXISFRAME(2,IBOX9,NEWBUFF)
                      DO J=1,NAXISFRAME(1,IBOX9,NEWBUFF)
                        IMAGEN(J+J1,I+I1,NEWBUFF+NMAXBUFF/2)=SQRT(
     +                  ABS(IMAGEN(J+J1,I+I1,NEWBUFF))/GAIN_ARRAY+
     +                  REAL(NCOADDS_ARRAY)*RNOISE_ARRAY*RNOISE_ARRAY)
                      END DO
                    END DO
     ...
     +         'x axis','signal',CDUMMY(1:L),3,201,1.0)
c..............................................................................
...
c..............................................................................
              ELSEIF(CINERR.EQ.'0')THEN
                ...
                END DO
c..............................................................................
C Para calcular los errores, es importante notar que el numero de coadds es un
C factor importante; de hecho, el ruido debido a la sen~al hay que calcularlo
C a partir de la sen~al en cada coadd, no a partir de la sen~al total integrada
C que, logicamente, es mucho mayor. Notar que en la formula que sigue, el
C factor NCOADSS_ARRAY que multiplica al error en cada coadd se cancela con
C el mismo factor que aparece al dividir la sen~al total para obtener la
C sen~al en cada coadd. Sin embargo, este factor no se cancela en el termino
C correspondiente al ruido de lectura.
     ...
     +               ABS(IMAGEN(J,I,NCBUFF))/GAIN_ARRAY+
     +               REAL(NCOADDS_ARRAY)*RNOISE_ARRAY*RNOISE_ARRAY)
                  END DO
                END DO
c..............................................................................
              ELSEIF(CINERR.EQ.'f')THEN
                ...
                DO WHILE(.NOT.LOGFILE)
                  ERRFILE=READC('Input FITS error file name',
     +             ERRFILE,'@')
                  IF((INDEX(ERRFILE,'*').NE.0).OR.
     +             (INDEX(ERRFILE,'?').NE.0))THEN
                    L1=TRUEBEG(ERRFILE)
                    L2=TRUELEN(ERRFILE)
                    ISYSTEM=
...
C------------------------------------------------------------------------------
     ...
     +       CSAVE,'frx')
            IF(CSAVE.NE.'x')THEN
              IF(NCBUFF.LE.NMAXBUFF/2)THEN
                CSAVEERR(1:1)=
     +           READC('Save error frame (y/n)',CSAVEERR,'yn')
              ELSE
                CSAVEERR='n'
              END IF
              DO WHILE(LOGFILE)
     ...
     +             '*.fits','@')
                ELSE
                  OUTFILE=READC('Output REDUCEME file name (none=EXIT)',
     +             '*.u','@')
                END IF
     ...
     +                 ERRFILE,'@')
                    ELSE
                      ERRFILE=READC('Output REDUCEME error file name',
     +                 ERRFILE,'@')
                    END IF
                    IF((INDEX(ERRFILE,'*').NE.0).OR.
     +               (INDEX(ERRFILE,'?').NE.0))THEN
...
C------------------------------------------------------------------------------
          ELSEIF(NB.EQ.4)THEN
            ...
            ELSE
              ...
              NYFINDAL=READILIM('Y grid looking for spectral lines',
     +         CDUMMY,1,NXYMAX/4)
              WRITE(CDUMMY,*) NINLINE
              NINLINE=READILIM('No. of peaks/line found',
     +         CDUMMY,1,NYFINDAL)
              WRITE(CDUMMY,*) NWIDTH
              NWIDTH=READILIM('No. of pixels to search for peaks (odd)',
     +         CDUMMY,3,NWIDTHMAX)
...
C------------------------------------------------------------------------------
          ELSEIF(NB.EQ.5)THEN
            ...
            IF(LOK)THEN
              ...
              IF(LPMAPP) CALL PMAPP
              IF(LPWAVE) CALL PWAVE(NDEGWV,CWV)
ccc              CALL STATISTIC(NCBUFF,NX1,NX2,NY1,NY2,
ccc               .FALSE.,.TRUE.,.TRUE.,0.0,.FALSE.)
            END IF
...
C------------------------------------------------------------------------------
          ELSEIF(NB.EQ.9)THEN
            ...
            LOGFILERR=.FALSE.
            DO WHILE(.NOT.LOGFILE)
              INFILE_=READC('Input file name with polynomial '//
     +         'wavelength calibration (none=EXIT)',
     +         'polw*','@')
              IF((INDEX(INFILE_,'*').NE.0).OR.
     +         (INDEX(INFILE_,'?').NE.0))THEN
                L1=TRUEBEG(INFILE_)
...
c..............................................................................
                ELSEIF((CDUM.EQ.'1').OR.(CDUM.EQ.'2'))THEN
                  NDEG=READILIM('Polynomial degree','0',0,
     +             MIN0(19,NSTACK-1))
                  IF(CDUM.EQ.'1')THEN
...
c..............................................................................
     ...
     +             MIN0(19,NSTACK-1))
                  ...
                  IF(CREFINE.EQ.'y')THEN
                    ...
                    WRITE(CDUMMY,*) NWIDTH
                    NWIDTH=READILIM(
     +               'No. of pixels to search for peaks (odd)',
     +               CDUMMY,3,NWIDTHMAX)
                  ...
                  IF(CUPDATE.EQ.'y')THEN
                    ...
                    END DO
                    FSHIFT=READF('Shift in Y (+ upwards, - downwards)',
     +               '0.0')
                    COEFFBL(1,NLINBL)=COEFFBL(1,NLINBL)+FSHIFT
                    LBOUNDL=(NLINBL.GE.2)
                    XMINBL(NLINBL)=0.
                    YMINBL(NLINBL)=FPOLY(NDEGBL(NLINBL),
     +               COEFFBL(1,NLINBL),XMINBL(NLINBL))
                    XMAXBL(NLINBL)=REAL(NAXIS(1,NCBUFF)+1)
                    YMAXBL(NLINBL)=FPOLY(NDEGBL(NLINBL),
     +               COEFFBL(1,NLINBL),XMAXBL(NLINBL))
                    CALL SORTLINES
                    WRITE(*,101) '***WARNING***'
...
c..............................................................................
     ...
     +               'No. of pixels to search for peaks (odd)',
     +               CDUMMY,3,NWIDTHMAX)
                    CALL REFINEBA(NDEG,COEFF,XSTACK(NSTACK/2),
     +               FMEAN_,FSIGMA_)
                    WRITE(*,101) '=> fit result: '
                    DO K=1,NDEG+1
                      WRITE(*,'(A2,I2,A3,$)') 'a(',K-1,'): '
...
C------------------------------------------------------------------------------
          ELSEIF(NB.EQ.11)THEN
            ...
            IF(CH.EQ.'Q')THEN
              LEXIT=.TRUE.
            ELSE
              CSURE(1:1)=READC('Do you really want to exit (y/n)',
     +         '@','yn')
              IF(CSURE.EQ.'y')THEN
                LEXIT=.TRUE.
              ELSE
...
C------------------------------------------------------------------------------
     ...
     +       CDUMMY,3,NWIDTHMAX)
            ...
            DO WHILE(CH.NE.'X')
              ...
              ELSE
                ...
                CALL INTERSEC(NDEGBA00,COEFFBA00,
     ...
     +           XC,XMAX,YMAX)
                ...
                CALL PGSCI(1)
                CALL REFINEBA(NDEGBA00,COEFFBA00,XP(NXYMAX/2),
     +           FMEAN_,FSIGMA_)
...
C------------------------------------------------------------------------------
          ELSEIF(NB.EQ.20)THEN
            ...
            IF(CH.NE.'T')THEN
              WRITE(CDUMMY,*) NBOX
              NBOX=READILIM(
     +         'NBOX (-4=mask,-3=box9, -2=quad., -1=mouse, 0=keyboard)',
     +         CDUMMY,-4,MIN(NXMAX,NYMAX))
            END IF
            IF(NBOX.EQ.-4)THEN
              WRITE(CDUMMY,*) MASKVALUE
              MASKVALUE=READF('Image value to be ignored in statistics',
     +         CDUMMY)
              ...
              IF((NNX1.GE.1).AND.(NNX2.LE.NAXIS(1,NCBUFF)).AND.
     +           (NNY1.GE.1).AND.(NNY2.LE.NAXIS(2,NCBUFF)))THEN
                CALL PGSCI(3)
                ...
                CALL PGSCI(1)
                CALL STATISTIC(NCBUFF,NNX1,NNX2,NNY1,NNY2,
     +           .TRUE.,.TRUE.,.TRUE.,MASKVALUE,.TRUE.)
              END IF
          ...
          ELSEIF(NB.EQ.21)THEN
            ...
            DO K=1,NMAXBUFF
              IF((NAXIS(1,K).NE.0).AND.(NAXIS(2,K).NE.0))THEN
                ...
                ELSE
                  WRITE(*,'(A5,I1,A12,$)') 'err #',K-NMAXBUFF/2,
     +             ' <DEFINED>: '
                END IF
                L=TRUELEN(INFILE(K))
...
C------------------------------------------------------------------------------
          ELSEIF(NB.EQ.24)THEN
            ...
            IF(CH.EQ.'C')THEN
              COFFSET(1:1)=
     +         READC('Measure relative offsets in box-9 (y/n)',
     +         'n','yn')
            ELSE
              COFFSET='n'
            END IF
            IF(COFFSET.EQ.'n')THEN
              CALL CENTROID(NCBUFF,CH,XC_,YC_,.FALSE.,
              ...
              IF(LOK)THEN
                ...
                DO I=2,NFRAMES(NCBUFF)
                  IF(LOK)THEN
                    ...
                    DO WHILE(LMIDE)
                      WRITE(*,'(A,I1,A,$)') 
     +                 'Select the same object in frame #',I,'...'
                      ...
                      END IF
                      IF(LMIDE)THEN
                        CALL CENTROID(NCBUFF,'d',XC__,YC__,.FALSE.,
     +                   X0,Y0,SIGMAX,SIGMAY,BETA,AMP,CTE,
     ...
     +                   .FALSE.)
                ...
                IF(LOK)THEN
                  ...
                  OPEN(10,FILE=OFFFILE,STATUS='UNKNOWN',
     +             FORM='FORMATTED')
...
C------------------------------------------------------------------------------
          ELSEIF(NB.EQ.25)THEN
            ...
            DO WHILE((NEWBUFF.EQ.NCBUFF).AND.(NEWBUFF.NE.0))
              ...
              WRITE(CDUMMY,*) NEWBUFF
              NEWBUFF=
     +         READILIM('Buffer # to store rectified image (0=none)',
     +         CDUMMY,0,NMAXBUFF)
              IF(NEWBUFF.EQ.NCBUFF)THEN
                WRITE(*,101) '***ERROR***'
                WRITE(*,101) '=> New buffer must be different than '//
     +           'current buffer. Try again.'
              END IF
            END DO
            IF(NEWBUFF.NE.0)THEN
              IF(IRESAMPL.EQ.1)THEN
                IRESAMPL=READILIM(
     +           'Resampling mode (1=approx., 2=accurate)','1',1,2)
              ELSE
                IRESAMPL=READILIM(
     +           'Resampling mode (1=approx., 2=accurate)','2',1,2)
              END IF
...
C------------------------------------------------------------------------------
          ELSEIF(NB.EQ.26)THEN
            ...
            DO WHILE(LASK)
              ...
              IF(NEWBUFF.GT.NMAXBUFF/2) NEWBUFF=1
              WRITE(CDUMMY,*) NEWBUFF
              NEWBUFF=READILIM('Buffer # to store intermediate data',
     +         CDUMMY,0,NMAXBUFF)
            ...
            DO WHILE(LASK)
              ...
              NEWBUFF_=READILIM('Buffer #2 to store intermediate data',
     +         CDUMMY,0,NMAXBUFF)
...
C------------------------------------------------------------------------------
          ELSEIF(NB.EQ.28)THEN
            ...
            IF((NAXIS(1,NCBUFF).NE.(J2-J1)).OR.
     +         (NAXIS(2,NCBUFF).NE.(I2-I1)))THEN
              WRITE(*,101) '***ERROR***'
            ...
            ELSE
              ...
              DO WHILE(LASK)
                ...
                IF(NEWBUFF.GT.NMAXBUFF/2) NEWBUFF=1
                WRITE(CDUMMY,*) NEWBUFF
                NEWBUFF=READILIM('Buffer # to store intermediate image',
     +           CDUMMY,0,NMAXBUFF)
                LASK=(NEWBUFF.EQ.NCBUFF)
                IF(LASK)THEN
                  WRITE(*,101) '***ERROR***'
...
C------------------------------------------------------------------------------
          ELSEIF(NB.EQ.30)THEN
            ...
            DO WHILE(.NOT.LOGFILE)
              FILELISTIN=
     +         READC('Input file name with list of FITS files','@','@')
            ...
            DO WHILE(LOGFILE)
              FILELISTOUT=
     +         READC('Output file name with list of FITS files','@','@')
...
882         CLOSE(10)
            ...
            DO I=1,NFITSFILES
              ...
              L1=TRUEBEG(FILEOFFSET)
              L2=TRUELEN(FILEOFFSET)
              CALL SLEEFITS(FILEOFFSET(L1:L2),.FALSE.,IROTATE,NCBUFF,
     +         .FALSE.,0)
              NFRAMES(NCBUFF)=1
              NSIZEFB9(NCBUFF)=0
...
C------------------------------------------------------------------------------
          ELSEIF(NB.EQ.31)THEN
            ...
            DAXIS1_=READI('Integer offset for X shift (+right,-left)',
     +       '0')
            DAXIS2_=READI('Integer offset for Y shift ...(+up,-down)',
     +       '0')
C inicializamos imagen de destino a cero
            DO I=1,NAXIS2_
              ...
              END DO
            END DO
C trasladamos los datos a la imagen de destino
            DO I=1,NAXIS(2,NCBUFF)
              ...
              END IF
            END DO
C pasamos los datos de la matriz temporal a la definitiva
            DO I=1,NAXIS2_
              DO J=1,NAXIS1_
                IMAGEN(J,I,NCBUFF)=IMAGEN_(J,I)
              END DO
            END DO
C Repetimos el trabajo con la imagen de errores
            IF(NCBUFF.LE.NMAXBUFF/2)THEN
              DO I=1,NAXIS2_
                DO J=1,NAXIS1_
                  IMAGEN_(J,I)=0.0
...
C------------------------------------------------------------------------------
          ELSEIF(NB.EQ.34)THEN
            ...
            IF(NBUFFBOX9.EQ.0)THEN
              NBUFFBOX9=READILIM('Buffer with set of box-9 frames '//
     +         '(0=exit)','@',0,NMAXBUFF)
            ELSE
              WRITE(CDUMMY,*) NBUFFBOX9
              NBUFFBOX9=READILIM('Buffer with set of box-9 frames '//
     +         '(0=exit)',CDUMMY,0,NMAXBUFF)
            END IF
          ...
          ELSEIF((NB.GE.35).AND.(NB.LE.34+NMAXBUFF/2))THEN
            ...
            ELSE
              ...
              DO K=NMAXBUFF/2+1,NMAXBUFF
                ...
                CALL BUTTON(44+K-NMAXBUFF/2,CDUMMY(1:L),0)
              END DO
              IF((NAXIS(1,NCBUFF).EQ.NAXIS(1,NCBUFF_)).AND.
     +           (NAXIS(2,NCBUFF).EQ.NAXIS(2,NCBUFF_)))THEN
                CALL SUBLOOK(.TRUE.,NCBUFF,.FALSE.)
              ELSE
                NX1=1
                NX2=NAXIS(1,NCBUFF)
...
C------------------------------------------------------------------------------
          ELSEIF(NB.EQ.44)THEN
            ...
            IF(NBUFFBOX9.EQ.0)THEN
              NBUFFBOX9=READILIM('Buffer with frames to be measured '//
     +         '(0=exit)','@',0,NMAXBUFF)
            ELSE
              WRITE(CDUMMY,*) NBUFFBOX9
              NBUFFBOX9=READILIM('Buffer with frames to be measured '//
...
C------------------------------------------------------------------------------
          ELSEIF((NB.GE.45).AND.(NB.LE.44+NMAXBUFF/2))THEN
            IF((NAXIS(1,NB-44+NMAXBUFF/2).EQ.0).AND.
     +         (NAXIS(2,NB-44+NMAXBUFF/2).EQ.0))THEN
              WRITE(*,101) '***WARNING***'
...
C ***END OF MAIN LOOP***
C------------------------------------------------------------------------------
C Close graphic display and end program
        CALL PGEND
        STOP
C------------------------------------------------------------------------------
100     FORMAT(A,$)
