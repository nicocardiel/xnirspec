        SUBROUTINE MIDEOFFSET(NFRAMES_,NEWBUFF1,NEWBUFF2,NEWBUFF3,
...
C------------------------------------------------------------------------------
        ...
        NSEED=-1
C------------------------------------------------------------------------------
C inicializamos buffer con posibles ajustes aceptados
        DO I=1,NYMAX
          DO J=1,NXMAX
...
C------------------------------------------------------------------------------
        ...
        DO WHILE(LOOP)
C..............................................................................
C mostramos parametros para los ajustes
          WRITE(*,*)
...
C hacemos un zoom alrededor del frame central
            NX1=DJ(1)*NSIZEFB9(NEWBUFF1)+1-NBOX_CENTROID
            NX2=DJ(1)*NSIZEFB9(NEWBUFF1)+NSIZEFB9(NEWBUFF1)-1+
     +       NBOX_CENTROID
            NY1=DI(1)*NSIZEFB9(NEWBUFF1)+1-NBOX_CENTROID
            NY2=DI(1)*NSIZEFB9(NEWBUFF1)+NSIZEFB9(NEWBUFF1)-1+
     +       NBOX_CENTROID
            ...
            IF(LPOST(CPOST,CBASEPOST,NPOST,IDOLD,IDNEW))THEN
              ...
              CALL PGSLCT(IDOLD)
            END IF
C seleccionamos punto inicial para ajustar un objeto del frame #1
            LOUTSIDE=.TRUE.
            DO WHILE(LOUTSIDE)
              WRITE(*,*)
     ...
     +            DI(1)*NSIZEFB9(NEWBUFF1)-NBOX_CENTROID/2-1)
            ...
            END DO
            IF(CH.EQ.'X')THEN
              CCHANGE(1:1)=READC('[c]hange fitting parameters or '//
     +         'e[x]it (c/x)','c','cx')
     ...
     +         5,MIN(NAXIS(1,NEWBUFF1),NAXIS(2,NEWBUFF1)))
              IF(MOD(NBOX_CENTROID,2).EQ.0)THEN
                NBOX_CENTROID=NBOX_CENTROID+1
                IF(NBOX_CENTROID.GT.
     +           MIN(NAXIS(1,NEWBUFF1),NAXIS(2,NEWBUFF1)))
     +           NBOX_CENTROID=NBOX_CENTROID-2
            ...
            ELSE
C sumamos todas las regiones disponibles para hacer un ajuste inicial que
C nos sirva para luego hacer los ajustes individuales de forma mas sencilla
C (nota: aqui hay que tener cuidado porque en los bordes del frame#1, hay
C zonas que no estan presentes en todas las imagenes del box-9)
              ...
              END DO
              DO K=1,NFRAMES_
                IX1=NINT(XC+FOFFSETX(K))-NBOX_CENTROID/2+
     +           DJ(K)*NSIZEFB9(NEWBUFF1)-DJ(1)*NSIZEFB9(NEWBUFF1)
                IX2=IX1+NBOX_CENTROID-1
                IY1=NINT(YC+FOFFSETY(K))-NBOX_CENTROID/2+
     ...
     +               DI(K)*NSIZEFB9(NEWBUFF1)))THEN
                      IMAGEN(J-IX1+1,I-IY1+1,NEWBUFF2)=
     +                 IMAGEN(J-IX1+1,I-IY1+1,NEWBUFF2)+
     +                 IMAGEN(J,I,NEWBUFF1)
                      IMAGEN_(J-IX1+1,I-IY1+1)=
     +                 IMAGEN_(J-IX1+1,I-IY1+1)+1.0
                    END IF
                  END DO
                END DO
              END DO
     ...
     +               IMAGEN_(J,I)
                ...
                END DO
              END DO
              CALL STATISTIC(NEWBUFF2,1,NBOX_CENTROID,1,NBOX_CENTROID,
...
C
              ...
              CALL PGWNAD(REAL(1)-0.6,REAL(NBOX_CENTROID)+0.6,
     +                    REAL(1)-0.6,REAL(NBOX_CENTROID)+0.6)
              CALL PGIMAG(IMAGEN(1,1,NEWBUFF2),NXMAX,NYMAX,
     +         1,NBOX_CENTROID,1,NBOX_CENTROID,FG,BG,TR)
...
C
              DO I=1,NBOX_CENTROID
                DO J=1,NBOX_CENTROID
                  FACTOR1=(REAL(J)-X0_)*(REAL(J)-X0_)/
     +             (2.*SIGMAX_*SIGMAX_)
                  FACTOR2=(REAL(I)-Y0_)*(REAL(I)-Y0_)/
     +             (2.*SIGMAY_*SIGMAY_)
                  FACTOR3=BETA_*(REAL(J)-X0_)*(REAL(I)-Y0_)/
     +             (SIGMAX_*SIGMAY_)
              ...
              CALL PGWNAD(REAL(1)-0.6,REAL(NBOX_CENTROID)+0.6,
     +                    REAL(1)-0.6,REAL(NBOX_CENTROID)+0.6)
              CALL PGIMAG(IMAGEN_,NXMAX,NYMAX,1,NBOX_CENTROID,
     +         1,NBOX_CENTROID,FG,BG,TR)
              CALL PGBOX('BCTS',0.0,0,'BCTS',0.0,0)
              CALL PGSCI(7)
              ...
              CALL PGSCH(OLD_CH)
              WRITE(*,100) 'Press <left/right> button to '//
     +         '<confirm/repeat>...'
...
C..............................................................................
          ...
          END IF
          CALL HISTOGRAM(NEWBUFF1)
C deshacemos un zoom y mostramos de nuevo toda la imagen
          NX1=1
...
C ajustamos centroides
     ...
     +          DJ(K)*NSIZEFB9(NEWBUFF1)-DJ(1)*NSIZEFB9(NEWBUFF1),
     +         YC+FOFFSETY(K)+
     +          DI(K)*NSIZEFB9(NEWBUFF1)-DI(1)*NSIZEFB9(NEWBUFF1),2)
              CALL PGSCI(1)
              IF(K.EQ.9)THEN
                CMODE='D'
              ELSE
                CMODE='d'
              END IF
              X0(NOBJFIT,K)=X0_-REAL(NBOX_CENTROID/2)+
     +         XC+FOFFSETX(K)+
     +         DJ(K)*NSIZEFB9(NEWBUFF1)-DJ(1)*NSIZEFB9(NEWBUFF1)
              Y0(NOBJFIT,K)=Y0_-REAL(NBOX_CENTROID/2)+
     +         YC+FOFFSETY(K)+
     +         DI(K)*NSIZEFB9(NEWBUFF1)-DI(1)*NSIZEFB9(NEWBUFF1)
...
C mostramos ajustes
     ...
     +         DI(K)*NSIZEFB9(NEWBUFF1)-DI(1)*NSIZEFB9(NEWBUFF1)
              ...
              DO I=IY1,IY2
                DO J=IX1,IX2
                  IF(INSIDE(J,1,NAXIS(1,NEWBUFF1),
     +             I,1,NAXIS(2,NEWBUFF1)))THEN
                    IMAGEN(J-IX1+1+DJJ,I-IY1+1+DII,NEWBUFF2)=
     +               IMAGEN(J,I,NEWBUFF1)
                  END IF
                END DO
              END DO
            END IF
          END DO
          CALL STATISTIC(NEWBUFF2,1,3*NBOX_CENTROID,1,3*NBOX_CENTROID,
     +     .FALSE.,.FALSE.,.TRUE.,0.0,.FALSE.)
          IF(FSIGMA.GT.0.0)THEN
            BG=FMEAN-5.*FSIGMA
            FG=FMEAN+5.*FSIGMA
          ...
          CALL PGIMAG(IMAGEN(1,1,NEWBUFF2),NXMAX,NYMAX,
     +     1,NAXIS(1,NEWBUFF2),1,NAXIS(2,NEWBUFF2),FG,BG,TR)
          CALL PGBOX('BCTS',0.0,0,'BCTS',0.0,0)
          CALL PGSCI(7)
     ...
     +         DI(K)*NSIZEFB9(NEWBUFF1)-DI(1)*NSIZEFB9(NEWBUFF1)
              ...
              DO I=IY1,IY2
                DO J=IX1,IX2
                  FACTOR1=(REAL(J)-X0(NOBJFIT,K))*
     +             (REAL(J)-X0(NOBJFIT,K))/
     +             (2.*SIGMAX(NOBJFIT,K)*SIGMAX(NOBJFIT,K))
                  FACTOR2=(REAL(I)-Y0(NOBJFIT,K))*
     +             (REAL(I)-Y0(NOBJFIT,K))/
     +             (2.*SIGMAY(NOBJFIT,K)*SIGMAY(NOBJFIT,K))
                  FACTOR3=BETA(NOBJFIT,K)*
     +             (REAL(J)-X0(NOBJFIT,K))*(REAL(I)-Y0(NOBJFIT,K))/
     +             (SIGMAX(NOBJFIT,K)*SIGMAY(NOBJFIT,K))
                  FACTOR=FACTOR1+FACTOR2+FACTOR3
                  IF(FACTOR.GT.60.)THEN
                    IMAGEN(J-IX1+1+DJJ,I-IY1+1+DII,NEWBUFF3)=
     +               CTE(NOBJFIT,K)
                  ELSE
                    IMAGEN(J-IX1+1+DJJ,I-IY1+1+DII,NEWBUFF3)=
     +               CTE(NOBJFIT,K)+AMP(NOBJFIT,K)*EXP(-FACTOR)
                  END IF
                END DO
              END DO
            END IF
          ...
          CALL PGSVP(0.21,0.39,0.40,0.79)
          CALL PGWNAD(XMIN,XMAX,YMIN,YMAX)
          CALL PGIMAG(IMAGEN(1,1,NEWBUFF3),NXMAX,NYMAX,
     +     1,NAXIS(1,NEWBUFF3),1,NAXIS(2,NEWBUFF3),FG,BG,TR)
          CALL PGBOX('BCTS',0.0,0,'BCTS',0.0,0)
          CALL PGSCI(7)
          ...
          CALL PGSCH(OLD_CH)
C..............................................................................
C calculamos offsets medios y dispersion
          DO K=1,NFRAMES_
            IF(LFITOK(NOBJFIT,K))THEN
              FOFFSETX_(NOBJFIT,K)=X0(NOBJFIT,K)-X0(NOBJFIT,1)-
     +         DJ(K)*NSIZEFB9(NEWBUFF1)+DJ(1)*NSIZEFB9(NEWBUFF1)-
     +         FOFFSETX(K)
              EFOFFSETX_(NOBJFIT,K)=EX0(NOBJFIT,K)
              FOFFSETY_(NOBJFIT,K)=Y0(NOBJFIT,K)-Y0(NOBJFIT,1)-
     +         DI(K)*NSIZEFB9(NEWBUFF1)+DI(1)*NSIZEFB9(NEWBUFF1)-
     +         FOFFSETY(K)
          ...
          DO K=1,NFRAMES_
            ...
            IF(NOBJEFF.GT.0)THEN
              ...
              FOFFSETX_WM(K)=FMEAN0W(NOBJEFF,XDATA,EXDATA,
     +         FOFFSETX_WS(K))
              FOFFSETY_M(K)=FMEAN0(NOBJEFF,YDATA,FOFFSETY_S(K))
              FOFFSETY_WM(K)=FMEAN0W(NOBJEFF,YDATA,EYDATA,
     +         FOFFSETY_WS(K))
              WRITE(*,100) 'Offsets > '
              WRITE(*,*) K,FOFFSETX_M(K)+FOFFSETX(K),FOFFSETX_S(K),
     +                     FOFFSETY_M(K)+FOFFSETY(K),FOFFSETY_S(K)
              WRITE(*,100) 'OffsetsW> '
              WRITE(*,*) K,FOFFSETX_WM(K)+FOFFSETX(K),FOFFSETX_WS(K),
     +                     FOFFSETY_WM(K)+FOFFSETY(K),FOFFSETY_WS(K)
            ELSE
              FOFFSETX_M(K)=0.
              FOFFSETX_S(K)=0.
              FOFFSETX_WM(K)=0.
            ...
            END IF
          END DO
C dibujamos offsets medidos hasta ahora
          CALL PGQVP(0,XV1,XV2,YV1,YV2)
          CALL PGQWIN(XW1,XW2,YW1,YW2)
          CALL PGQCH(OLD_CH)
          IF(CPOST.EQ.'y')THEN
          ...
          DO NPLOT=1,NPLOT2
            IF(NPLOT.EQ.1)THEN !en X11
              ...
              DO K=1,NFRAMES_
                DO NO=1,NOBJFIT
                  IF(LFITOK(NO,K))THEN
                    IF(YMINOFF.GT.FOFFSETX_(NO,K)-EFOFFSETX_(NO,K)) 
     +                YMINOFF=FOFFSETX_(NO,K)-EFOFFSETX_(NO,K)
                    IF(YMINOFF.GT.FOFFSETY_(NO,K)-EFOFFSETY_(NO,K)) 
     +                YMINOFF=FOFFSETY_(NO,K)-EFOFFSETY_(NO,K)
                    IF(YMAXOFF.LT.FOFFSETX_(NO,K)+EFOFFSETX_(NO,K)) 
     +                YMAXOFF=FOFFSETX_(NO,K)+EFOFFSETX_(NO,K)
                    IF(YMAXOFF.LT.FOFFSETY_(NO,K)+EFOFFSETY_(NO,K)) 
     +                YMAXOFF=FOFFSETY_(NO,K)+EFOFFSETY_(NO,K)
                  END IF
                END DO
                IF(YMINOFF.GT.FOFFSETX_M(K)-FOFFSETX_S(K)) 
     +            YMINOFF=FOFFSETX_M(K)-FOFFSETX_S(K)
                IF(YMINOFF.GT.FOFFSETX_WM(K)-FOFFSETX_WS(K)) 
     +            YMINOFF=FOFFSETX_WM(K)-FOFFSETX_WS(K)
                IF(YMAXOFF.LT.FOFFSETX_M(K)+FOFFSETX_S(K)) 
     +            YMAXOFF=FOFFSETX_M(K)+FOFFSETX_S(K)
                IF(YMAXOFF.LT.FOFFSETX_WM(K)+FOFFSETX_WS(K)) 
     +            YMAXOFF=FOFFSETX_WM(K)+FOFFSETX_WS(K)
              END DO
              ...
              IF(K.LE.NFRAMES_)THEN
                DO NO=1,NOBJFIT
                  IF(LFITOK(NO,K))THEN
                    ...
                    CALL PGERRY(1,REAL(NO)-0.2,
     +               FOFFSETX_(NO,K)-EFOFFSETX_(NO,K),
     +               FOFFSETX_(NO,K)+EFOFFSETX_(NO,K),1.0)
                    CALL PGSCI(2)
                    CALL PGPOINT(1,REAL(NO)+0.2,FOFFSETY_(NO,K),1)
                    CALL PGERRY(1,REAL(NO)+0.2,
     +               FOFFSETY_(NO,K)-EFOFFSETY_(NO,K),
     +               FOFFSETY_(NO,K)+EFOFFSETY_(NO,K),1.0)
                    CALL PGSCI(1)
                  END IF
                END DO
                IF(NOBJFIT.GT.1)THEN
                  ...
                  CALL PGERRY(1,-2.0-0.2,
     +             FOFFSETX_M(K)-FOFFSETX_S(K),
     +             FOFFSETX_M(K)+FOFFSETX_S(K),1.0)
                  CALL PGPOINT(1,-1.0-0.2,FOFFSETX_WM(K),1)
                  CALL PGERRY(1,-1.0-0.2,
     +             FOFFSETX_WM(K)-FOFFSETX_WS(K),
     +             FOFFSETX_WM(K)+FOFFSETX_WS(K),1.0)
                  CALL PGSCI(2)
                  CALL PGPOINT(1,-2.0+0.2,FOFFSETY_M(K),1)
                  CALL PGERRY(1,-2.0+0.2,
     +             FOFFSETY_M(K)-FOFFSETY_S(K),
     +             FOFFSETY_M(K)+FOFFSETY_S(K),1.0)
                  CALL PGPOINT(1,-1.0+0.2,FOFFSETY_WM(K),1)
                  CALL PGERRY(1,-1.0+0.2,
     +             FOFFSETY_WM(K)-FOFFSETY_WS(K),
     +             FOFFSETY_WM(K)+FOFFSETY_WS(K),1.0)
                  CALL PGSCI(1)
                END IF
              END IF
            END DO
          ...
          CALL PGSCH(OLD_CH)
C..............................................................................
C si el numero de objetos ajustados es mayor o igual a 3, podemos calcular
C las correspondientes transformaciones afines (de esta manera podemos estimar
C la importancia de una posible distorsion/rotacion). Tomamos como referencia
C los objetos del frame #1 (notar que el error en los puntos del frame #1 se
C los pasamos a la subrutina de calculo de la transformacion afin como si
C fueran errores de la imagen K-esima, con K>1).
            ...
            DO K=2,NFRAMES_ !para los demas frames que no son el #1
              ...
              DO NO=1,NOBJFIT
                IF(LFITOK(NO,K))THEN
                  ...
                  UDATA(NOBJEFF)=X0(NO,K)-
     +             DJ(K)*NSIZEFB9(NEWBUFF1)+DJ(1)*NSIZEFB9(NEWBUFF1)-
     +             FOFFSETX(K)
                  EUDATA(NOBJEFF)=EX0(NO,K)
                  VDATA(NOBJEFF)=Y0(NO,K)-
     +             DI(K)*NSIZEFB9(NEWBUFF1)+DI(1)*NSIZEFB9(NEWBUFF1)-
     +             FOFFSETY(K)
              ...
              IF(NOBJEFF.GE.1)THEN !en la imagen K-esima hay al menos 1 punto
                CALL AFINE_TRA(NOBJEFF,
     +           XDATA,YDATA,UDATA,VDATA,
     +           A11,A21,A31,A12,A22,A32,CHISQR)
                IF(NSIMUL.GT.0)THEN
                  DO NS=1,NSIMUL
                    DO NO=1,NOBJEFF
                      R1=RANDOMNUMBER(NSEED)
                      R2=RANDOMNUMBER(NSEED)
                      ERRORX=1.41421356*EXDATA(NO)*
     +                 SQRT(-1.*LOG(1.-R1))*COS(2.*PI*R2)
                      XDATA_(NO)=XDATA(NO)+ERRORX
                      R1=RANDOMNUMBER(NSEED)
                      R2=RANDOMNUMBER(NSEED)
                      ERRORY=1.41421356*EYDATA(NO)*
     +                 SQRT(-1.*LOG(1.-R1))*COS(2.*PI*R2)
                      YDATA_(NO)=YDATA(NO)+ERRORY
                      R1=RANDOMNUMBER(NSEED)
                      R2=RANDOMNUMBER(NSEED)
                      ERRORU=1.41421356*EUDATA(NO)*
     +                 SQRT(-1.*LOG(1.-R1))*COS(2.*PI*R2)
                      UDATA_(NO)=UDATA(NO)+ERRORU
                      R1=RANDOMNUMBER(NSEED)
                      R2=RANDOMNUMBER(NSEED)
                      ERRORV=1.41421356*EVDATA(NO)*
     +                 SQRT(-1.*LOG(1.-R1))*COS(2.*PI*R2)
                      VDATA_(NO)=VDATA(NO)+ERRORV
                    END DO
                    CALL AFINE_TRA(NOBJEFF,
     +               XDATA_,YDATA_,UDATA_,VDATA_,
     +               A11_(NS),A21_(NS),A31_(NS),
     +               A12_(NS),A22_(NS),A32_(NS),CHISQR_(NS))
                  END DO
...
C..............................................................................
            ...
            IF(9*NBOX_CENTROID.LE.NXMAX/3)THEN
              DO I=1,3*NBOX_CENTROID
                DO J=1,3*NBOX_CENTROID
                  IMAGEN(J,I+NAXIS(2,NEWBUFF4),NEWBUFF4)=
     +             IMAGEN(J,I,NEWBUFF2)
                END DO
              END DO
              DO I=1,3*NBOX_CENTROID
                DO J=1,3*NBOX_CENTROID
                  IMAGEN(J+3*NBOX_CENTROID,I+NAXIS(2,NEWBUFF4),NEWBUFF4)
     +              =IMAGEN(J,I,NEWBUFF3)
                END DO
              END DO
              DO I=1,3*NBOX_CENTROID
                DO J=1,3*NBOX_CENTROID
                  IMAGEN(J+6*NBOX_CENTROID,I+NAXIS(2,NEWBUFF4),NEWBUFF4)
     +              =IMAGEN(J,I,NEWBUFF2)-IMAGEN(J,I,NEWBUFF3)
                END DO
              END DO
          ...
          ELSE
            ...
            IF(NOBJFIT.GT.0)THEN
              WRITE(*,*)
              WRITE(*,101) 'Recomputing offsets after removing'//
     +          ' last fit...'
              DO K=1,NFRAMES_
                NOBJEFF=0
                DO NO=1,NOBJFIT
     ...
     +                         FOFFSETY_M(K)+FOFFSETY(K),FOFFSETY_S(K)
                  WRITE(*,100) 'OffsetsW> '
                  WRITE(*,*) K,FOFFSETX_WM(K)+FOFFSETX(K),
     +                         FOFFSETX_WS(K),
     +                         FOFFSETY_WM(K)+FOFFSETY(K),
     +                         FOFFSETY_WS(K)
                ELSE
                  FOFFSETX_M(K)=0.
                  FOFFSETX_S(K)=0.
...
C..............................................................................
        END DO
C------------------------------------------------------------------------------
C si no tenemos ningun ajuste, regresamos
        IF(NOBJFIT.EQ.0)THEN
          RETURN
        END IF
C------------------------------------------------------------------------------
C damos la posibilidad de salvar los offsets calculados
        DO K=1,NFRAMES_
     ...
     +                 FOFFSETY_WM(K)+FOFFSETY(K),FOFFSETY_WS(K)
        END DO
        CSAVE(1:1)=
     +   READC('Save these measured offsets into a file (y/n)',
     +   'n','yn')
        IF(CSAVE.EQ.'y')THEN
          CWEIGHT(1:1)=
     +     READC('Save [w]eighted or [u]nweighted fits (w/u)',
     +     'w','wu')
          LOGFILE=.TRUE.
          DO WHILE(LOGFILE)
          ...
          IF(CWEIGHT.EQ.'w')THEN
            DO K=1,NFRAMES_
              WRITE(30,*) K,FOFFSETX_WM(K)+FOFFSETX(K),FOFFSETX_WS(K),
     +                      FOFFSETY_WM(K)+FOFFSETY(K),FOFFSETY_WS(K)
            END DO
          ELSE
            DO K=1,NFRAMES_
              WRITE(30,*) K,FOFFSETX_M(K)+FOFFSETX(K),FOFFSETX_S(K),
     +                      FOFFSETY_M(K)+FOFFSETY(K),FOFFSETY_S(K)
            END DO
          END IF
          CLOSE(30)
        END IF
C------------------------------------------------------------------------------
C redibujamos botones del histograma
...
C
C******************************************************************************
C Calcula la transformacion afin general (translation+rotation+scale+shear) 
C usando NOBJEFF puntos. La solucion se obtiene resolviendo dos sistemas de 
C ecuaciones con tres incognitas (para NOBJEFF>3 el sistema esta 
C sobredeterminado).
...
C------------------------------------------------------------------------------
        ...
        DO I=1,N
          ...
          SUMY=SUMY+Y(I)
        END DO
C resolvemos el problema aplicando la regla de Cramer
        DETERMINANTE=DETER3P3(SUMUU,SUMUV,SUMU,
     +                        SUMUV,SUMVV,SUMV,
     +                        SUMU,SUMV,REAL(N))
        IF(DETERMINANTE.GE.1.E-6)THEN
          A11=DETER3P3(SUMXU,SUMUV,SUMU,
     +                 SUMXV,SUMVV,SUMV,
     +                 SUMX,SUMV,REAL(N))/DETERMINANTE
          A21=DETER3P3(SUMUU,SUMXU,SUMU,
     +                 SUMUV,SUMXV,SUMV,
     +                 SUMU,SUMX,REAL(N))/DETERMINANTE
          A31=DETER3P3(SUMUU,SUMUV,SUMXU,
     +                 SUMUV,SUMVV,SUMXV,
     +                 SUMU,SUMV,SUMX)/DETERMINANTE
          A12=DETER3P3(SUMYU,SUMUV,SUMU,
     +                 SUMYV,SUMVV,SUMV,
     +                 SUMY,SUMV,REAL(N))/DETERMINANTE
          A22=DETER3P3(SUMUU,SUMYU,SUMU,
     +                 SUMUV,SUMYV,SUMV,
     +                 SUMU,SUMY,REAL(N))/DETERMINANTE
          A32=DETER3P3(SUMUU,SUMUV,SUMYU,
     +                 SUMUV,SUMVV,SUMYV,
     +                 SUMU,SUMV,SUMY)/DETERMINANTE
        ELSE
...
C
        DO I=1,N
          CHISQR=CHISQR+
     +     (A11*U(I)+A21*V(I)+A31-X(I))*
     +     (A11*U(I)+A21*V(I)+A31-X(I))+
     +     (A12*U(I)+A22*V(I)+A32-Y(I))*
     +     (A12*U(I)+A22*V(I)+A32-Y(I))
        END DO
C------------------------------------------------------------------------------
100     FORMAT(A,$)
...
C
C******************************************************************************
C Calcula la transformacion afin traslacion pura.
...
C
C******************************************************************************
C Calcula la transformacion afin rotacion pura.
...
C
        A=1.0
        B=2.0*(SUMUU+SUMVV)
        C=(SUMUU+SUMVV)*(SUMUU+SUMVV)-(SUMXU+SUMYV)*(SUMXU+SUMYV)-
     +   (SUMYU-SUMXV)*(SUMYU-SUMXV)
        DELTA=B*B-4.0*A*C
...
C
        ...
        DO I=1,N
          CHISQR_=CHISQR_+
     +     (A11_*U(I)+A21_*V(I)+A31-X(I))*
     +     (A11_*U(I)+A21_*V(I)+A31-X(I))+
     +     (A12_*U(I)+A22_*V(I)+A32-Y(I))*
     +     (A12_*U(I)+A22_*V(I)+A32-Y(I))
        END DO
C
...
C
C******************************************************************************
C Calcula la transformacion afin de escala pura
...
C
C******************************************************************************
C Calcula la transformacion afin de shear en X
...
C
C******************************************************************************
C Calcula la transformacion afin de shear en Y
...
C
        ...
        REAL A31,A32,A33
C
        DETER3P3=A11*A22*A33+A21*A32*A13+A12*A23*A31-
