C Version May 8, 2000: 
C       Modifico un poco la subrutina para que los parametros
C       de entrada esten en formato real. El calculo sigue haciendose en doble
C       precision.
C******************************************************************************
C Ajusta un polinomio generico por minimos cuadrados (sin errores)
        SUBROUTINE FITPOL(N,X_,Y_,NDEG,IFCOEF,
     +   B_,VARB_,CHISQR_,SR2_,COVAR_)
        IMPLICIT NONE
C
        INCLUDE 'largest.inc'
...
C------------------------------------------------------------------------------
        ...
        END IF
C------------------------------------------------------------------------------
C pasamos los datos a doble precision
        DO K=1,N
          X(K)=DBLE(X_(K))
          Y(K)=DBLE(Y_(K))
        END DO
...
C introducimos las varianzas calculadas en la matriz de salida
        ...
        END DO
C------------------------------------------------------------------------------
C las variables de salida no estan en doble precision
        DO I=1,NDEG+1
          IF(IFCOEF(I))THEN
            B_(I)=REAL(B(I))
            IF(VARB(I).LE.1.E-30)THEN
              VARB_(I)=0.
ccc las 2 lineas anteriores evitan: Floating point exception 5, underflow
            ELSE
              VARB_(I)=REAL(VARB(I))
            END IF
          ELSE
