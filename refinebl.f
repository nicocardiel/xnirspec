C Refina la posicion de una linea BL del boundary.
C Nota: el polinomio de entrada en la subrutina es modificados a la salida.
C XMIN,XMAX: es el recorrido de la variable X que va a ser examinado.
...
C en lugar de dibujar el ajuste directamente es mejor mostrar los residuos
        ...
        IF(LDEBUGLOCAL)THEN
          ...
          YMIN_=-1.
          YMAX_=+1.
          CALL SUBPLOT(NPEAKS,1,NPEAKS,XPEAKS,RPEAKS,XPEAKS,RPEAKS,
     +     .TRUE.,.FALSE.,.FALSE.,.FALSE.,
     +     'wavelength direction','residuals in spatial direction',' ',
     +     3,1,1.5)
          J=0
          DO I=1,NPEAKS
            IF(.NOT.LFIT(I))THEN
            ...
            END IF
          END DO
          IF(J.GT.0) CALL SUBPLOTBIS(J,1,J,XF,YF,XF,YF,
     +     .FALSE.,.FALSE.,2,5,1.0)
          DO I=1,NXYMAX
          ...
          CALL PGSCI(1)
          CALL SUBPLOTBIS(NXYMAX,1,NXYMAX,XF,YF,XF,YF,
