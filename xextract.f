...
ccc        REAL SPLA(NXYMAX),SPLB(NXYMAX),SPLC(NXYMAX),SPLS(NXYMAX)
...
C
          ...
          DO J=J1,J2
            ...
            NF=I2-I1+1
            IF(CMORE.EQ.'y')THEN
              CALL SUBPLOT(NF,1,NF,XF,YF,XF,YF,
     +         .TRUE.,.TRUE.,.FALSE.,.FALSE.,
     +         'y-axis','signal',' ',3,201,1.0)
            END IF
c
              IF(INDEX('1467',CFIT).NE.0)THEN
              CALL GAUSSFIT(NF,XF,YF,YF,X0,SIGMA,AMP,
     +         EX0,ESIGMA,EAMP,EEX0,EESIGMA,EEAMP,1.E-6,0)
...
c
            IF(INDEX('2457',CFIT).NE.0)THEN
              CALL GAUSSFITA(NF,XF,YF,YF,X0,SIGMA,AMP,
     +         EX0,ESIGMA,EAMP,EEX0,EESIGMA,EEAMP,1.E-6,0)
...
c
            IF(INDEX('3567',CFIT).NE.0)THEN
              CALL GAUSCFIT(NF,XF,YF,X0,SIGMA,AMP,Y0,
     +         EEX0,EESIGMA,EEAMP,EEY0,1.E-6)
     ...
     +            .FALSE.,.FALSE.,5,101,1.0)
          ...
          DO WHILE(CMORE.EQ.'y')
            IF(INDEX('1467',CFIT).NE.0)THEN
              CALL SUBPLOT(NPEAKS,1,NPEAKS,XPEAK,YPEAK1,XPEAK,EYPEAK1,
     +         .TRUE.,.TRUE.,.FALSE.,.TRUE.,
     +         'X axis','\\gDY',' ',3,1,1.0)
            END IF
            IF(INDEX('2457',CFIT).NE.0)THEN
              IF(INDEX('47',CFIT).NE.0)THEN
                CALL SUBPLOTBIS(NPEAKS,1,NPEAKS,XPEAK,YPEAK2,XPEAK,
     +           EYPEAK2,.FALSE.,.TRUE.,4,1,1.0)
              ELSE
                CALL SUBPLOT(NPEAKS,1,NPEAKS,XPEAK,YPEAK2,XPEAK,EYPEAK2,
     +           .TRUE.,.TRUE.,.FALSE.,.TRUE.,
     +           'X axis','\\gDY',' ',3,1,1.0)
              END IF
            END IF
            IF(INDEX('3567',CFIT).NE.0)THEN
              IF(CFIT.EQ.'3')THEN
                CALL SUBPLOT(NPEAKS,1,NPEAKS,XPEAK,YPEAK3,XPEAK,EYPEAK3,
     +           .TRUE.,.TRUE.,.FALSE.,.TRUE.,
     +           'X axis','\\gDY',' ',3,1,1.0)
              ELSE
                CALL SUBPLOTBIS(NPEAKS,1,NPEAKS,XPEAK,YPEAK3,XPEAK,
     +           EYPEAK3,.FALSE.,.TRUE.,5,1,1.0)
              END IF
             END IF
...
C------------------------------------------------------------------------------
C------------------------------------------------------------------------------
C NDEG,COEFF0 es el polinomio correctivo que aplicamos a NMAP,COEFFLOCAL para
C determinar el centro del perfil espacial.
C------------------------------------------------------------------------------
C------------------------------------------------------------------------------
C Dibujamos el nuevo centro del objeto
        DO J=1,NXYMAX
          U=UMIN+REAL(J-1)/REAL(NXYMAX-1)*(UMAX-UMIN)
          CALL FMAP(NMAP,AIJ_,BIJ_,U,VINI,XP(J),YP(J))
        ...
        CALL PGLINE(NXYMAX,XP,YP)
        CALL PGSCI(1)
C NCOADD se refiere a pixels en el sistema distorsionado, no en la matriz
C original de datos.
        NCOADD=READI('Number of spectra at each side of maximum','@')
C Dibujamos la region a sumar
        DO J=1,NXYMAX
          U=UMIN+REAL(J-1)/REAL(NXYMAX-1)*(UMAX-UMIN)
        ...
        IF(LERR)THEN
          IF(NAXIS(1,NEWBUFF+NMAXBUFF/2).LT.J2-J1)
     +     NAXIS(1,NEWBUFF+NMAXBUFF/2)=J2-J1
          IF(NAXIS(2,NEWBUFF+NMAXBUFF/2).LT.NEWSCAN)
     +     NAXIS(2,NEWBUFF+NMAXBUFF/2)=NEWSCAN
        END IF
C------------------------------------------------------------------------------
C Extraccion no optimizada
C------------------------------------------------------------------------------
...
ccc              IF(I.EQ.0)THEN
ccc                CALL PPOLY4(X1,Y1,X2,Y2,X3,Y3,X4,Y4,2)
ccc              ELSE
ccc                CALL PPOLY4(X1,Y1,X2,Y2,X3,Y3,X4,Y4,3)
ccc              END IF
              XORIG=X1
              YORIG=Y1
              XCOR2(1,1)=0.
              ...
              DO II=IYMIN,IYMAX
                DO JJ=IXMIN,IXMAX
                  IF((II.GE.1).AND.(II.LE.NAXIS(2,NCBUFF)).AND.
     +               (JJ.GE.1).AND.(JJ.LE.NAXIS(1,NCBUFF)))THEN
                    X1=REAL(JJ)-0.5
                    X2=X1+1.
                    Y1=REAL(II)-0.5
                    Y2=Y1+1.
ccc                 CALL PPOLY4(X1,Y1,X2,Y1,X2,Y2,X1,Y2,6)
                    XCOR1(1,1)=X1-XORIG
                    XCOR1(2,1)=Y1-YORIG
                    XCOR1(1,2)=X2-XORIG
                    XCOR1(2,2)=XCOR1(2,1)
...
ccc                 CALL PPOLYN(4,XCOR1,FAREATOT)
     ...
     +                   FAREA/FAREATOT
            ...
            IF(LERR)THEN
              IMAGEN(J-J1+1,NEWSCAN,NEWBUFF+NMAXBUFF/2)=
     +         SQRT(IMAGEN(J-J1+1,NEWSCAN,NEWBUFF+NMAXBUFF/2))
            END IF
...
C Extraccion optimizada
        ...
        READ(*,*)
C------------------------------------------------------------------------------
C Definimos el recorrido espectral a utilizar (usamos coordenadas en el sistema
C de referencia del espectro distorsionado; es lo que en el programa llamo
C U range).
        DO J=1,NXMAX
          IFU(J)=.FALSE.
        END DO
        ...
        DO WHILE(LOOP)
          IF(K.EQ.0)THEN
            CALL READ2I('U range (0,0=exit)',
     +       '1,'//CDUMMY(1:L2),J1_,J2_)
          ELSE
        ...
        END DO
C..............................................................................
C Definimos el subpixelado a emplear. Este factor es tenido en cuenta por
C la variable FACTORFLUX que garantiza que el corte promedio tenga unidades
C similares a un corte promedio de la imagen.
...
C..............................................................................
C Las variables XOPTIMAL,YOPTIMAL van a almacenar el corte promedio en la
C direccion espacial con el subpixelado elegido.
        DO IOPT=1,NSPLFIT
          XOPTIMAL(IOPT)=-REAL(NCOADD)-0.5+
     +     (REAL(IOPT)-0.5)/REAL(NSUBPIX)
          YOPTIMAL(IOPT)=0.
        END DO
C..............................................................................
C Calculamos el corte espacial promedio.
        NEXTINFO=0
        K_=0
        DO J=J1,J2-1
...
ccc                        CALL PPOLYN(4,XCOR1,FAREATOT)
                        ...
                        IF(NCOR.GT.0)THEN
                          CALL PPOLYN(NCOR,XCOM,FAREA)
                          YOPTIMAL(IOPT)=YOPTIMAL(IOPT)+
     +                     IMAGEN(JJ,II,NCBUFF)*
     +                     FAREA/FAREATOT*FACTORFLUX
                        END IF
                      END IF
                    END DO
                  END DO
        ...
        DO WHILE(LOOP)
          ...
          DO I=1,NKNOTS
            XKNOT(I)=XOPTIMAL(1)+
     +       REAL(I-1)/REAL(NKNOTS-1)*(XOPTIMAL(NSPLFIT)-XOPTIMAL(1))
          END DO
          YRMSTOL=READF('YRMSTOL for downhill','1.E-6')
          CALL PGSCI(2)
          CALL SPLFIT(NSPLFIT,XOPTIMAL,YOPTIMAL,NKNOTS,XKNOT,YRMSTOL,
     +     NXYMAX,XP,YP,XOPTIMAL(1),XOPTIMAL(NSPLFIT),FRMS,.TRUE.)
          CALL PGSCI(1)
          CMORE(1:1)=READC('Another fit (y/n)','n','yn')
          LOOP=(CMORE.EQ.'y')
        END DO
