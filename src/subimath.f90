...
! elegimos primer buffer
          ...
          END IF
!..............................................................................
! mostramos posibles operaciones
          ...
          END IF
!..............................................................................
! elegimos region de la imagen inicial a utilizar (salvo si estamos copiando
! un buffer completo o haciendo una transformacion 1 <--> 4 o 1 <--> 9, en 
! cuyo caso no hay nada que elegir)
          IF(.NOT.LCANCEL)THEN
            CALL BUTTON(NORIGEN+02,'[f]ull frame',0)
            CALL BUTTON(NORIGEN+12,'[r]egion',0)
            LCONT=.FALSE.
            IF((COPER.EQ.'<').OR.(COPER.EQ.'4').OR.(COPER.EQ.'1').OR.(COPER.EQ.'9').OR.(COPER.EQ.'c').OR.(COPER.EQ.'q'))THEN
              NX1=0
              NX2=0
              NY1=0
              NY2=0
          ...
          END IF
!..............................................................................
! la operacion '0' puede hacerse ya
          IF(.NOT.LCANCEL)THEN
            IF(COPER.EQ.'0')THEN
              DO I=NY1,NY2
                DO J=NX1,NX2
          ...
          END IF
!..............................................................................
! pedimos segundo elemento de la operacion (salvo si la operacion es copiar
! frame, filtrar frame, shift, 1 <--> 4 o s <--> 9, en cuyo caso esta opcion es
! inmediata)
          ...
          END IF
!..............................................................................
! NOTA: las operaciones con las imagenes de errores deben hacerse antes que 
! las operaciones con las imagenes de datos, para  evitar que al redefinir la
! matriz de datos, esto provoque que perdamos informacion que luego 
! necesitariamos para calcular las matrices de errores.
!..............................................................................
! la operacion con una constante ya puede hacerse
          IF(.NOT.LCANCEL)THEN
            IF(CUTIL.EQ.'t')THEN
              LASK=.TRUE.
              DO WHILE(LASK)
                  ...
                  DO I=NY1,NY2
                    DO J=NX1,NX2
                      IMAGEN(J,I,NBUFF0+NMAXBUFF/2)=SQRT( &
                       IMAGEN(J,I,NBUFF0+NMAXBUFF/2)*IMAGEN(J,I,NBUFF0+NMAXBUFF/2)+ &
                       FCTE_ERR*FCTE_ERR)
                    END DO
                  END DO
                END IF
              ...
              ELSEIF(COPER.EQ.'*')THEN
                IF(NBUFF0.LE.NMAXBUFF/2)THEN
                  DO I=NY1,NY2
                    DO J=NX1,NX2
                      IMAGEN(J,I,NBUFF0+NMAXBUFF/2)=SQRT( &
                       IMAGEN(J,I,NBUFF0)*IMAGEN(J,I,NBUFF0)*FCTE_ERR*FCTE_ERR+ &
                       IMAGEN(J,I,NBUFF0+NMAXBUFF/2)*IMAGEN(J,I,NBUFF0+NMAXBUFF/2)*FCTE*FCTE)
                    END DO
                  END DO
                END IF
                DO I=NY1,NY2
                ...
                IF(NBUFF0.LE.NMAXBUFF/2)THEN
                  DO I=NY1,NY2
                    DO J=NX1,NX2
                      IMAGEN(J,I,NBUFF0+NMAXBUFF/2)=SQRT( &
                       IMAGEN(J,I,NBUFF0)*IMAGEN(J,I,NBUFF0)*FCTE_ERR*FCTE_ERR+ &
                       IMAGEN(J,I,NBUFF0+NMAXBUFF/2)*IMAGEN(J,I,NBUFF0+NMAXBUFF/2)*FCTE*FCTE)/(FCTE*FCTE)
                    END DO
                  END DO
                END IF
                DO I=NY1,NY2
          ...
          END IF
!..............................................................................
! elegimos segundo buffer (OJO: solo dejamos elegir un buffer ya definido y
! que ademas tenga las mismas dimensiones que el buffer inicial ---esta ultima
! condicion no es exigible cuando estemos copiando un buffer ya definido en 
! uno vacio, cuando estamos filtrando una imagen, cuando comprimimos/expandimos
! un box-9 o cuando hagamos shift)
          IF(.NOT.LCANCEL)THEN
            DO I=1,NMAXBUFF
              NB=10*(I-1)+NORIGEN+3
              IF(I.LE.NMAXBUFF/2)THEN
              ...
              L=TRUELEN(CDUMMY)
              CALL BUTTON(NB,CDUMMY(1:L),0)
              IF((COPER.EQ.'<').OR.(COPER.EQ.'f').OR.(COPER.EQ.'4').OR.(COPER.EQ.'1').OR. &
                 (COPER.EQ.'9').OR.(COPER.EQ.'c').OR.(COPER.EQ.'s').OR.(COPER.EQ.'q'))THEN
                LDEFBUFF(I)=((NAXIS(1,I).NE.0).AND.(NAXIS(2,I).NE.0))
              ELSE
                LDEFBUFF(I)=((NAXIS(1,I).EQ.NAXIS(1,NBUFF0)).AND.(NAXIS(2,I).EQ.NAXIS(2,NBUFF0)))
              END IF
            ...
            DO WHILE(.NOT.LCONT)
              ...
              ELSE
                DO I=1,NMAXBUFF
                  IF((NB.EQ.NORIGEN+(I-1)*10+03).AND.(LDEFBUFF(I).OR.(COPER.EQ.'f').OR.(COPER.EQ.'s')))THEN
                    IF(COPER.EQ.'9')THEN
...
! la operacion '<' puede hacerse ya
              ...
              IF((NBUFF0.LE.NMAXBUFF/2).AND.(NBUFF1.LE.NMAXBUFF/2))THEN
                ...
                DO I=1,NAXIS(2,NBUFF0+NMAXBUFF/2)
                  DO J=1,NAXIS(1,NBUFF0+NMAXBUFF/2)
                    IMAGEN(J,I,NBUFF0+NMAXBUFF/2)=IMAGEN(J,I,NBUFF1+NMAXBUFF/2)
                  END DO
                END DO
              END IF
              LCANCEL=.TRUE.
          ...
          END IF
!..............................................................................
! la operacion '4' puede hacerse ya
          IF(.NOT.LCANCEL)THEN
            IF(COPER.EQ.'4')THEN
              NAXIS(1,NBUFF0)=NAXIS(1,NBUFF1)
              NAXIS(2,NBUFF0)=NAXIS(2,NBUFF1)
              ...
              IF((NBUFF0.LE.NMAXBUFF/2).AND.(NBUFF1.LE.NMAXBUFF/2))THEN
                ...
                DO I=1,NMED2
                  DO J=1,NMED1
                    IMAGEN(J,I,NBUFF0+NMAXBUFF/2)=IMAGEN(2*J-1,2*I-1,NBUFF1+NMAXBUFF/2)
                    IMAGEN(J+NMED1,I,NBUFF0+NMAXBUFF/2)=IMAGEN(2*J,2*I-1,NBUFF1+NMAXBUFF/2)
                    IMAGEN(J,I+NMED2,NBUFF0+NMAXBUFF/2)=IMAGEN(2*J-1,2*I,NBUFF1+NMAXBUFF/2)
                    IMAGEN(J+NMED1,I+NMED2,NBUFF0+NMAXBUFF/2)=IMAGEN(2*J,2*I,NBUFF1+NMAXBUFF/2)
                  END DO
                END DO
              END IF
              LCANCEL=.TRUE.
          ...
          END IF
!..............................................................................
! la operacion '1' puede hacerse ya
          IF(.NOT.LCANCEL)THEN
            IF(COPER.EQ.'1')THEN
              NAXIS(1,NBUFF0)=NAXIS(1,NBUFF1)
              NAXIS(2,NBUFF0)=NAXIS(2,NBUFF1)
              ...
              IF((NBUFF0.LE.NMAXBUFF/2).AND.(NBUFF1.LE.NMAXBUFF/2))THEN
                ...
                DO I=1,NMED2
                  DO J=1,NMED1
                    IMAGEN(2*J-1,2*I-1,NBUFF0+NMAXBUFF/2)=IMAGEN(J,I,NBUFF1+NMAXBUFF/2)
                    IMAGEN(2*J,2*I-1,NBUFF0+NMAXBUFF/2)=IMAGEN(J+NMED1,I,NBUFF1+NMAXBUFF/2)
                    IMAGEN(2*J-1,2*I,NBUFF0+NMAXBUFF/2)=IMAGEN(J,I+NMED2,NBUFF1+NMAXBUFF/2)
                    IMAGEN(2*J,2*I,NBUFF0+NMAXBUFF/2)=IMAGEN(J+NMED1,I+NMED2,NBUFF1+NMAXBUFF/2)
                  END DO
                END DO
              END IF
              LCANCEL=.TRUE.
          ...
          END IF
!..............................................................................
! la operacion '9' puede hacerse ya
          IF(.NOT.LCANCEL)THEN
            IF(COPER.EQ.'9')THEN
          ...
          END IF
!..............................................................................
! la operacion 'c' puede hacerse ya
          IF(.NOT.LCANCEL)THEN
            IF(COPER.EQ.'c')THEN
              ...
              DO I=1,NAXIS(2,NBUFF1)
                DO J=1,NAXIS(1,NBUFF1)
                  IMAGEN(J,I,NBUFF0)=0.
                  DO K=1,9
                    IMAGEN(J,I,NBUFF0)=IMAGEN(J,I,NBUFF0)+IMAGEN(J+DJ(K),I+DI(K),NBUFF1)
                  END DO
          ...
          END IF
!..............................................................................
! la operacion 'q' puede hacerse ya
          IF(.NOT.LCANCEL)THEN
            IF(COPER.EQ.'q')THEN
              NAXIS(1,NBUFF0)=NAXIS(1,NBUFF1)
              NAXIS(2,NBUFF0)=NAXIS(2,NBUFF1)
              ...
              DO I=1,NMED2
                DO J=1,NMED1
                  IMAGEN(J+NOFFX0,I+NOFFY0,NBUFF0)=IMAGEN(J+NOFFX1,I+NOFFY1,NBUFF1)
                END DO
              END DO
              IF((NBUFF0.LE.NMAXBUFF/2).AND.(NBUFF1.LE.NMAXBUFF/2))THEN
                NAXIS(1,NBUFF0+NMAXBUFF/2)=NAXIS(1,NBUFF1+NMAXBUFF/2)
                ...
                DO I=1,NMED2
                  DO J=1,NMED1
                    IMAGEN(J+NOFFX0,I+NOFFY0,NBUFF0+NMAXBUFF/2)=IMAGEN(J+NOFFX1,I+NOFFY1,NBUFF1+NMAXBUFF/2)
                  END DO
                END DO
              END IF
              LCANCEL=.TRUE.
          ...
          END IF
!..............................................................................
! la operacion entre dos buffers (sin cortes en X o en Y) tambien puede hacerse
          IF(.NOT.LCANCEL)THEN
            IF(CUTIL.EQ.'f')THEN
              IF(COPER.EQ.'+')THEN
                IF((NBUFF0.LE.NMAXBUFF/2).AND.(NBUFF1.LE.NMAXBUFF/2))THEN
                  DO I=NY1,NY2
                    DO J=NX1,NX2
                      IMAGEN(J,I,NBUFF0+NMAXBUFF/2)=SQRT( &
                       IMAGEN(J,I,NBUFF0+NMAXBUFF/2)*IMAGEN(J,I,NBUFF0+NMAXBUFF/2)+ &
                       IMAGEN(J,I,NBUFF1+NMAXBUFF/2)*IMAGEN(J,I,NBUFF1+NMAXBUFF/2))
                    END DO
                  END DO
                END IF
                DO I=NY1,NY2
                  DO J=NX1,NX2
                    IMAGEN(J,I,NBUFF0)=IMAGEN(J,I,NBUFF0)+IMAGEN(J,I,NBUFF1)
                  END DO
                END DO
              ELSEIF(COPER.EQ.'-')THEN
                IF((NBUFF0.LE.NMAXBUFF/2).AND.(NBUFF1.LE.NMAXBUFF/2))THEN
              ...
              ELSEIF(COPER.EQ.'*')THEN
                IF((NBUFF0.LE.NMAXBUFF/2).AND.(NBUFF1.LE.NMAXBUFF/2))THEN
                  DO I=NY1,NY2
                    DO J=NX1,NX2
                      IMAGEN(J,I,NBUFF0+NMAXBUFF/2)=SQRT( &
                       IMAGEN(J,I,NBUFF0)*IMAGEN(J,I,NBUFF0)*IMAGEN(J,I,NBUFF1+NMAXBUFF/2)*IMAGEN(J,I,NBUFF1+NMAXBUFF/2)+ &
                       IMAGEN(J,I,NBUFF1)*IMAGEN(J,I,NBUFF1)*IMAGEN(J,I,NBUFF0+NMAXBUFF/2)*IMAGEN(J,I,NBUFF0+NMAXBUFF/2))
                    END DO
                  END DO
                END IF
                DO I=NY1,NY2
              ...
              ELSEIF(COPER.EQ.'/')THEN
                ...
                IF(LDIVZERO)THEN
                  ...
                  IF(CZERO.EQ.'c')THEN
                    ...
                    IF((NBUFF0.LE.NMAXBUFF/2).AND.(NBUFF1.LE.NMAXBUFF/2))THEN
                      FZERO_ERR=READF('Pixel value for error in infinity','0.0')
                      DO I=NY1,NY2
                        DO J=NX1,NX2
          ...
          END IF
!..............................................................................
! la operacion 'f' (filter) puede hacerse ya
          IF(.NOT.LCANCEL)THEN
            IF(COPER.EQ.'f')THEN
              WRITE(*,101) ' '
...
!
              ...
              IF((NBUFF0.LE.NMAXBUFF/2).AND.(NBUFF1.LE.NMAXBUFF/2))THEN
                ...
                END DO
              END IF
! set options for selected filter
              IF(CFILT.EQ.'0')THEN
              ...
              ELSEIF((CFILT.EQ.'4').OR.(CFILT.EQ.'5').OR.(CFILT.EQ.'6').OR.(CFILT.EQ.'7'))THEN
                ...
                ELSE
                  IWIDTH=READILIM('Window width (pixels; odd)',CDUMMY,3,NAXIS(2,NBUFF0))
                END IF
                ...
                IF((CFILT.EQ.'4').OR.(CFILT.EQ.'5'))THEN
                  ...
                  NDEGREE=READILIM('Polynomial degree',CDUMMY,0,MIN0(19,IWIDTH-1))
                  IF(CFILT.EQ.'5')THEN
                    WRITE(CDUMMY,*) TSIGMA
                    TSIGMA=READF('Times sigma to exclude points',CDUMMY)
                  END IF
...
! execute filter
              ...
              ELSEIF(CFILT.EQ.'m')THEN
                ...
                OFFSET_LOG=READF('Offset for logarithmic transformation', '0.0')
                ...
                DO I=NY1,NY2
                  DO J=NX1,NX2
                    FDUMMY=IMAGEN(J,I,NBUFF0)+OFFSET_LOG
                    IF(FDUMMY.GT.0.0)THEN
                      IMAGEN(J,I,NBUFF1)=ALOG10(IMAGEN(J,I,NBUFF0)+OFFSET_LOG)
                    ELSE
                    ...
                    DO II=I-IKERNEL,I+IKERNEL
                      DO JJ=J-IKERNEL,J+IKERNEL
                        IF((II.EQ.I).AND.(JJ.EQ.J))THEN
                          FDUMMY=FDUMMY+FLOAT(2*IKERNEL+1)*FLOAT(2*IKERNEL+1)*IMAGEN(JJ,II,NBUFF0)
                        ELSE
              ...
              ELSEIF(CFILT.EQ.'9')THEN
                DO I=NY1,NY2
                  DO J=NX1,NX2
                    IF((IMAGEN(J,I,NBUFF0).GE.THRESHOLD1).AND.(IMAGEN(J,I,NBUFF0).LE.THRESHOLD2))THEN
                    ...
                    DO J=NX1,NX2
                      ...
                      DO JJ=NX1_,NX2_
                        ...
                        ELSE
                          FACTOR=0.
                        END IF
                        IMAGEN(J,I,NBUFF1)=IMAGEN(J,I,NBUFF1)+IMAGEN(JJ,I,NBUFF0)*FACTOR
                    ...
                    DO I=NY1,NY2
                      ...
                      DO II=NY1_,NY2_
                        ...
                        ELSE
                          FACTOR=0.
                        END IF
                        IMAGEN(J,I,NBUFF1)=IMAGEN(J,I,NBUFF1)+IMAGEN(J,II,NBUFF0)*FACTOR
                        SUMFACTOR=SUMFACTOR+FACTOR
                      END DO
                      IMAGEN(J,I,NBUFF1)=IMAGEN(J,I,NBUFF1)/SUMFACTOR
                    END DO
                  ...
                  DO I=NY1,NY2
                    DO J=NX1,NX2
                      ...
                      IF(CFUN_OR_DER.EQ.'1')THEN !function
                        ...
                        IF(NCOEF.GT.1)THEN
                          DO K=NCOEF-1,1,-1
                            IMAGEN(J,I,NBUFF1)=IMAGEN(J,I,NBUFF1)*REAL(J)+COEF(K)
                          END DO
                        END IF
                      ELSE                       !derivative
                        ...
                        IF(NCOEF.GT.1)THEN
                          ...
                          IF(NCOEFDER.GT.1)THEN
                            DO K=NCOEFDER-1,1,-1
                              IMAGEN(J,I,NBUFF1)=IMAGEN(J,I,NBUFF1)*REAL(J)+COEFDER(K)
                            END DO
                          END IF
                        END IF
                      END IF
                  ...
                  DO J=NX1,NX2
                    DO I=NY1,NY2
                      ...
                      IF(CFUN_OR_DER.EQ.'1')THEN !function
                        ...
                        IF(NCOEF.GT.1)THEN
                          DO K=NCOEF-1,1,-1
                            IMAGEN(J,I,NBUFF1)=IMAGEN(J,I,NBUFF1)*REAL(I)+COEF(K)
                          END DO
                        END IF
                      ELSE                       !derivative
                        IMAGEN(J,I,NBUFF1)=0.0
                        IF(NCOEF.GT.1)THEN
                          ...
                          IF(NCOEFDER.GT.1)THEN
                            DO K=NCOEFDER-1,1,-1
                              IMAGEN(J,I,NBUFF1)=IMAGEN(J,I,NBUFF1)*REAL(I)+COEFDER(K)
                            END DO
                          END IF
                        END IF
                      END IF
              ...
              ELSE ! CFILT = 0, 1, 2 or 3
                WRITE(CDUMMY,*) NBOX_X
                NBOX_X=READILIM('Box width in X (must be odd)',CDUMMY,1,NAXIS(1,NBUFF0))
                ...
                END IF
                WRITE(CDUMMY,*) NBOX_Y
                NBOX_Y=READILIM('Box width in Y (must be odd)',CDUMMY,1,NAXIS(2,NBUFF0))
...
!
          ...
          END IF
!..............................................................................
! la operacion 's' (offset xy) puede hacerse ya
          IF(.NOT.LCANCEL)THEN
            IF(COPER.EQ.'s')THEN
              XOFFSET=READF('Offset for X shift (+right,-left)','0.0')
              YOFFSET=READF('Offset for Y shift ...(+up,-down)','0.0')
              J0=INT(XOFFSET)
              FJ0=XOFFSET-REAL(J0)
              I0=INT(YOFFSET)
              FI0=YOFFSET-REAL(I0)
! inicializamos imagen de destino a cero
              ...
              DO I=1,NAXIS(2,NBUFF1)
                ...
                END DO
              END DO
! si tenemos errores, inicializamos imagen de errores de destino a cero
              IF((NBUFF0.LE.NMAXBUFF/2).AND.(NBUFF1.LE.NMAXBUFF/2))THEN
                NAXIS(1,NBUFF1+NMAXBUFF/2)=NAXIS(1,NBUFF0+NMAXBUFF/2)
                NAXIS(2,NBUFF1+NMAXBUFF/2)=NAXIS(2,NBUFF0+NMAXBUFF/2)
                NFRAMES(NBUFF1+NMAXBUFF/2)=NFRAMES(NBUFF0+NMAXBUFF/2)
                ...
                DO J=1,NAXIS(1,NBUFF0)
                  ...
                  IF((J1.GE.1).AND.(J1.LE.NAXIS(1,NBUFF1)))THEN
                    IF((I1.GE.1).AND.(I1.LE.NAXIS(2,NBUFF1)))THEN
                      IMAGEN(J1,I1,NBUFF1)=IMAGEN(J1,I1,NBUFF1)+IMAGEN(J,I,NBUFF0)*(1.-ABS(FI0))*(1.-ABS(FJ0))
                    END IF
                    IF((I2.GE.1).AND.(I2.LE.NAXIS(2,NBUFF1)))THEN
                      IMAGEN(J1,I2,NBUFF1)=IMAGEN(J1,I2,NBUFF1)+IMAGEN(J,I,NBUFF0)*ABS(FI0)*(1.-ABS(FJ0))
                    END IF
                  END IF
                  IF((J2.GE.1).AND.(J2.LE.NAXIS(1,NBUFF1)))THEN
                    IF((I1.GE.1).AND.(I1.LE.NAXIS(2,NBUFF1)))THEN
                      IMAGEN(J2,I1,NBUFF1)=IMAGEN(J2,I1,NBUFF1)+IMAGEN(J,I,NBUFF0)*(1.-ABS(FI0))*ABS(FJ0)
                    END IF
                    IF((I2.GE.1).AND.(I2.LE.NAXIS(2,NBUFF1)))THEN
                      IMAGEN(J2,I2,NBUFF1)=IMAGEN(J2,I2,NBUFF1)+IMAGEN(J,I,NBUFF0)*ABS(FI0)*ABS(FJ0)
                    END IF
                  END IF
                END DO
              END DO
! si tenemos errores, aplicamos el offset a la imagen de errores
              IF((NBUFF0.LE.NMAXBUFF/2).AND.(NBUFF1.LE.NMAXBUFF/2))THEN
                DO I=1,NAXIS(2,NBUFF0+NMAXBUFF/2)
                  I1=I+I0
                  IF(FI0.EQ.0.0)THEN
                  ...
                  DO J=1,NAXIS(1,NBUFF0+NMAXBUFF/2)
                    ...
                    END IF
                    IF((J1.GE.1).AND.(J1.LE.NAXIS(1,NBUFF1)))THEN
                      IF((I1.GE.1).AND.(I1.LE.NAXIS(2,NBUFF1+NMAXBUFF/2)))THEN
                        IMAGEN(J1,I1,NBUFF1+NMAXBUFF/2)=IMAGEN(J1,I1,NBUFF1+NMAXBUFF/2)+ &
                         IMAGEN(J,I,NBUFF0+NMAXBUFF/2)*IMAGEN(J,I,NBUFF0+NMAXBUFF/2)*(1.-ABS(FI0))*(1.-ABS(FJ0))
                      END IF
                      IF((I2.GE.1).AND.(I2.LE.NAXIS(2,NBUFF1+NMAXBUFF/2)))THEN
                        IMAGEN(J1,I2,NBUFF1+NMAXBUFF/2)=IMAGEN(J1,I2,NBUFF1+NMAXBUFF/2)+ &
                         IMAGEN(J,I,NBUFF0+NMAXBUFF/2)*IMAGEN(J,I,NBUFF0+NMAXBUFF/2)*ABS(FI0)*(1.-ABS(FJ0))
                      END IF
                    END IF
                    IF((J2.GE.1).AND.(J2.LE.NAXIS(1,NBUFF1)))THEN
                      IF((I1.GE.1).AND.(I1.LE.NAXIS(2,NBUFF1+NMAXBUFF/2)))THEN
                        IMAGEN(J2,I1,NBUFF1+NMAXBUFF/2)=IMAGEN(J2,I1,NBUFF1+NMAXBUFF/2)+ &
                         IMAGEN(J,I,NBUFF0+NMAXBUFF/2)*IMAGEN(J,I,NBUFF0+NMAXBUFF/2)*(1.-ABS(FI0))*ABS(FJ0)
                      END IF
                      IF((I2.GE.1).AND.(I2.LE.NAXIS(2,NBUFF1+NMAXBUFF/2)))THEN
                        IMAGEN(J2,I2,NBUFF1+NMAXBUFF/2)=IMAGEN(J2,I2,NBUFF1+NMAXBUFF/2)+ &
                         IMAGEN(J,I,NBUFF0+NMAXBUFF/2)*IMAGEN(J,I,NBUFF0+NMAXBUFF/2)*ABS(FI0)*ABS(FJ0)
                      END IF
                    END IF
                  END DO
                END DO
                DO I=1,NAXIS(2,NBUFF1+NMAXBUFF/2)
                  DO J=1,NAXIS(1,NBUFF1+NMAXBUFF/2)
                    IMAGEN(J,I,NBUFF1+NMAXBUFF/2)=SQRT(IMAGEN(J,I,NBUFF1+NMAXBUFF/2))
                  END DO
                END DO
              END IF
! ya hemos terminado
...
! calculamos corte en X
          IF(.NOT.LCANCEL)THEN
            IF(CUTIL.EQ.'x')THEN
              ...
              DO WHILE(LASK)
                WRITE(CDUMMY,*) NY1_PLOT
                NY1_=READILIM('Y min (pixel, 0=EXIT)',CDUMMY,0,NAXIS(2,NBUFF1))
                LASK=(NY1_.NE.0)
                IF(LASK)THEN
                  WRITE(CDUMMY,*) NY2_PLOT
                  NY2_=READILIM('Y max (pixel)',CDUMMY,NY1_,NAXIS(2,NBUFF1))
                  LASK=(NY2_.NE.0)
                  IF(LASK)THEN
                    DO I=NY1_,NY2_
                      IFSCAN(I)=.TRUE.
              ...
              IF(.NOT.LCANCEL)THEN
                DO J=1,NAXIS(1,NBUFF1)
                  ...
                  ELSEIF(ICUT.EQ.2)THEN
                    XCUT(J)=FMEAN2E(K,YCUT,YCUT_ERR,3.0,FSIGMA,XCUT_ERR(J))
                    ...
                    DO I=NY1,NY2
                      DO J=NX1,NX2
                        IMAGEN(J,I,NBUFF0+NMAXBUFF/2)= &
                         SQRT(IMAGEN(J,I,NBUFF0+NMAXBUFF/2)*IMAGEN(J,I,NBUFF0+NMAXBUFF/2)+XCUT_ERR(J)*XCUT_ERR(J))
                      END DO
                    END DO
                  END IF
                  DO I=NY1,NY2
                ...
                ELSEIF(COPER.EQ.'*')THEN
                  IF(NBUFF0.LE.NMAXBUFF/2)THEN
                    DO I=NY1,NY2
                      DO J=NX1,NX2
                        IMAGEN(J,I,NBUFF0+NMAXBUFF/2)=SQRT( &
                         IMAGEN(J,I,NBUFF0+NMAXBUFF/2)*IMAGEN(J,I,NBUFF0+NMAXBUFF/2)*XCUT(J)*XCUT(J)+ &
                         IMAGEN(J,I,NBUFF0)*IMAGEN(J,I,NBUFF0)*XCUT_ERR(J)*XCUT_ERR(J))
                      END DO
                    END DO
                  END IF
                  DO I=NY1,NY2
                ...
                ELSEIF(COPER.EQ.'/')THEN
                  ...
                  IF(LDIVZERO)THEN
                    ...
                    IF(CZERO.EQ.'c')THEN
                      FZERO=READF('Pixel value for infinity','0.0')
                      IF(NBUFF0.LE.NMAXBUFF/2)THEN
                        FZERO_ERR=READF('Pixel value for error at infinity','0.0')
                        ...
                        ELSE
                          DO I=NY1,NY2
                            DO J=NX1,NX2
                              IMAGEN(J,I,NBUFF0+NMAXBUFF/2)=SQRT( &
                               IMAGEN(J,I,NBUFF0+NMAXBUFF/2)*IMAGEN(J,I,NBUFF0+NMAXBUFF/2)*XCUT(J)*XCUT(J)+ &
                               IMAGEN(J,I,NBUFF0)*IMAGEN(J,I,NBUFF0)*XCUT_ERR(J)*XCUT_ERR(J))/(XCUT(J)*XCUT(J))
                            END DO
                          END DO
                        END IF
                      END IF
...
! calculamos corte en Y
          IF(.NOT.LCANCEL)THEN
            IF(CUTIL.EQ.'y')THEN
              ...
              DO WHILE(LASK)
                WRITE(CDUMMY,*) NX1_PLOT
                NX1_=READILIM('X min (pixel, 0=EXIT)',CDUMMY,0,NAXIS(1,NBUFF1))
                LASK=(NX1_.NE.0)
                IF(LASK)THEN
                  WRITE(CDUMMY,*) NX2_PLOT
                  NX2_=READILIM('X max (pixel)',CDUMMY,NX1_,NAXIS(1,NBUFF1))
                  LASK=(NX2_.NE.0)
                  IF(LASK)THEN
                    DO J=NX1_,NX2_
                      IFCHAN(J)=.TRUE.
              ...
              IF(.NOT.LCANCEL)THEN
                DO I=1,NAXIS(2,NBUFF1)
                  ...
                  ELSEIF(ICUT.EQ.2)THEN
                    YCUT(I)=FMEAN2E(K,XCUT,XCUT_ERR,3.0,FSIGMA,YCUT_ERR(I))
                  ELSEIF(ICUT.EQ.3)THEN
                ...
                IF(COPER.EQ.'+')THEN
                  IF(NBUFF0.LE.NMAXBUFF/2)THEN
                    DO I=NY1,NY2
                      DO J=NX1,NX2
                        IMAGEN(J,I,NBUFF0+NMAXBUFF/2)= &
                         SQRT(IMAGEN(J,I,NBUFF0+NMAXBUFF/2)*IMAGEN(J,I,NBUFF0+NMAXBUFF/2)+YCUT_ERR(I)*YCUT_ERR(I))
                      END DO
                    END DO
                  END IF
                  DO I=NY1,NY2
                ...
                ELSEIF(COPER.EQ.'*')THEN
                  IF(NBUFF0.LE.NMAXBUFF/2)THEN
                    DO I=NY1,NY2
                      DO J=NX1,NX2
                        IMAGEN(J,I,NBUFF0+NMAXBUFF/2)=SQRT( &
                         IMAGEN(J,I,NBUFF0+NMAXBUFF/2)*IMAGEN(J,I,NBUFF0+NMAXBUFF/2)*YCUT(I)*YCUT(I)+ &
                         IMAGEN(J,I,NBUFF0)*IMAGEN(J,I,NBUFF0)*YCUT_ERR(I)*YCUT_ERR(I))
                      END DO
                    END DO
                  END IF
                  DO I=NY1,NY2
                        ...
                        DO I=NY1,NY2
                          ...
                          ELSE
                            DO J=NX1,NX2
                              IMAGEN(J,I,NBUFF0+NMAXBUFF/2)=SQRT( &
                               IMAGEN(J,I,NBUFF0+NMAXBUFF/2)*IMAGEN(J,I,NBUFF0+NMAXBUFF/2)*YCUT(I)*YCUT(I)+ &
                               IMAGEN(J,I,NBUFF0)*IMAGEN(J,I,NBUFF0)*YCUT_ERR(I)*YCUT_ERR(I))/(YCUT(I)*YCUT(I))
                            END DO
                          END IF
                        END DO
                      END IF
...
!..............................................................................
        END DO
!------------------------------------------------------------------------------
! desactivamos todos los botones en la region de la calculadora
