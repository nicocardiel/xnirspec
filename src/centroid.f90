! Calcula el centroide en una region seleccionada con el raton
! Si CH='d', la posicion no se lee del raton sino que se toma de los
! parametros de entrada XC,YC. El resultado del ajuste y los errores
! correspondientes son parametros de salida de la subrutina.
! Si LGUESS=.TRUE., los parametros X0_F,Y0_F,SIGMAX_F,SIGMAY_F,BETA_F,AMP_F,
! y CTE_F son un ajuste preliminar de los datos.
! Si LSHOW=.TRUE. muestra resultados intermedios.
...
!
        IF(CH.EQ.'C')THEN
          ...
          NBOX_CENTROID=READILIM('Box size in pixels (odd)',CDUMMY,5,MIN(NAXIS(1,NBUFF),NAXIS(2,NBUFF)))
          IF(MOD(NBOX_CENTROID,2).EQ.0)THEN
            NBOX_CENTROID=NBOX_CENTROID+1
            IF(NBOX_CENTROID.GT.MIN(NAXIS(1,NBUFF),NAXIS(2,NBUFF))) NBOX_CENTROID=NBOX_CENTROID-2
          END IF
        ...
        DO WHILE(.NOT.LEXIT)
          ...
          IF(CH_.NE.'X')THEN
            ...
            DO I=IY1,IY2
              ...
              END DO
            END DO
! calculo sencillo del centro de masas (usando momentos)
! (ver https://en.wikipedia.org/wiki/Image_moment)
            M00 = 0.0
            M10 = 0.0
            M01 = 0.0
            DO I=IY1,IY2
...
! almacenamos region de dibujo actual
            ...
            CALL PGQCH(OLD_CH)
            CALL PGSCH(0.7)
! borramos el plot previo
            CALL RPGERASW(0.00,0.40,0.00,0.80,0)
! definimos nueva region de dibujo para el dibujo 3D y lo hacemos
            CALL PGSVP(0.01,0.19,0.39,0.79)
            CALL PLOT3DBARS(IX1,IX2,IY1,IY2,0,.TRUE.,BG,FG)
! definimos nueva region de dibujo para el dibujo 2D y lo hacemos
            CALL PGSVP(0.0275,0.1425,0.21,0.38)
            CALL PGWNAD(REAL(IX1)-0.6,REAL(IX2)+0.6,REAL(IY1)-0.6,REAL(IY2)+0.6)
            CALL PGBOX('BC',0.0,0,'BC',0.0,0)
            CALL PGIMAG(IMAGEN(1,1,NBUFF),NXMAX,NYMAX,IX1,IX2,IY1,IY2,FG,BG,TR)
            CALL PGSCI(7)
            CALL PGMTXT('T',-1.5,0.5,0.5,'original data')
            CALL PGSCI(1)
! si tenemos un ajuste inicial, lo pasamos a GAUSS2DFIT
            IF(LGUESS)THEN
              X0=X0_F
              Y0=Y0_F
              SIGMAX=SIGMAX_F
...
! calculamos nueva region ajustada
            DO I=IY1,IY2
              DO J=IX1,IX2
              ...
              END DO
            END DO
! dibujamos resultados del ajuste 1D: calculamos limites
            NPFITX=IX2-IX1+1
            DO J=IX1,IX2
              XP(J-IX1+1)=REAL(J)
              YP(J-IX1+1)=0.
...
! corte promedio en Y
            ...
            CALL PGBIN(NPFITY,XP_,YP_,.TRUE.)
            CALL PGSCI(1)
! mostramos resultados del ajuste
            IF(LSHOW)THEN
              WRITE(*,100) 'NPFIT: '
              ...
              WRITE(*,*) AMP,EAMP,EEAMP,(AMPX_*2.5*SIGMAY_+AMPY_*2.5*SIGMAX_)/2.
              ...
              END IF
            END IF
! almacenamos valores ajustados como parametros de salida de la subrutina
            X0_F=X0
            Y0_F=Y0
            SIGMAX_F=SIGMAX
            SIGMAY_F=SIGMAY
...
! definimos nueva region de dibujo para el dibujo 2D y lo hacemos
            ...
            CALL PGWNAD(REAL(IX1)-0.6,REAL(IX2)+0.6,REAL(IY1)-0.6,REAL(IY2)+0.6)
            CALL PGBOX('BC',0.0,0,'BC',0.0,0)
            CALL PGIMAG(IMAGEN_,NXYMAX,NXYMAX,IX1,IX2,IY1,IY2,FG,BG,TR)
            CALL PGSCI(7)
...
! calculamos residuos
            DO I=IY1,IY2
              ...
              END DO
            END DO
! calculamos media, r.m.s., y dibujamos residuos
            FMEAN=0.
            DO I=IY1,IY2
              DO J=IX1,IX2
            ...
            CALL PGWNAD(REAL(IX1)-0.6,REAL(IX2)+0.6,REAL(IY1)-0.6,REAL(IY2)+0.6)
            CALL PGBOX('BC',0.0,0,'BC',0.0,0)
            CALL PGIMAG(IMAGEN_,NXYMAX,NXYMAX,IX1,IX2,IY1,IY2,FG_,BG_,TR)
            CALL PGSCI(7)
...
!
!******************************************************************************
! Funcion a integrar (aqui Y es constante, y la unica variable es X)
