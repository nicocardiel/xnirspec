...
C
C Escribe en ficheros las imagenes individuales del box-9 desplazadas en
C funcion de los offsets introducidos a traves de un fichero. De esta forma
C las imagenes individuales pueden sumarse posteriormente.
...
C proteccion
        ...
        END IF
C------------------------------------------------------------------------------
C definimos el buffer de trabajo
        NEWBUFF1=NCBUFF+1
        IF(NEWBUFF1.GT.NMAXBUFF/2) NEWBUFF1=1
        WRITE(CDUMMY,*) NEWBUFF1
        NEWBUFF1=READILIM(
     +   'Auxiliary buffer # to store manipulated data',
     +   CDUMMY,1,NMAXBUFF/2)
C------------------------------------------------------------------------------
C indicamos el nombre base de los ficheros de salida
        LOGFILE=.TRUE.
        DO WHILE(LOGFILE)
...
C------------------------------------------------------------------------------
        DO KFRAME=1,NFRAMES_
          ...
          END DO
C..............................................................................
C recorremos cada pixel en la nueva imagen y determinamos cuantos pixels del
C frame considerado se pueden usar
          DO I=1,NAXIS(2,NEWBUFF1)
            DO J=1,NAXIS(1,NEWBUFF1)
              ...
              DO IJ=1,4 !recorremos los 4 posibles pixels
                IF(IJ.EQ.1)THEN
                  JJ=JJ1
                  II=II1
                  FRACCION_PIXEL=(1.-ABS(FJ0(KFRAME)))*
     +                           (1.-ABS(FI0(KFRAME)))
                ELSEIF(IJ.EQ.2)THEN
                  JJ=JJ1
                  II=II2
     ...
     +                           (1.-ABS(FI0(KFRAME)))
                ...
                END IF
                IF(INSIDE(JJ,1,NAXISFRAME(1,KFRAME,NCBUFF),
     +                    II,1,NAXISFRAME(2,KFRAME,NCBUFF)))THEN
                  IF(IMAGEN(JJ+DJ(KFRAME)*NSIZEFB9(NCBUFF),
     +             II+DI(KFRAME)*NSIZEFB9(NCBUFF),NCBUFF+NMAXBUFF/2).GE.
     +             0.0)THEN
                    KK=KK+1
                    PIXEL(KK)=IMAGEN(JJ+DJ(KFRAME)*NSIZEFB9(NCBUFF),
     +                               II+DI(KFRAME)*NSIZEFB9(NCBUFF),
     +                               NCBUFF)
                    EPIXEL(KK)=
     +               IMAGEN(JJ+DJ(KFRAME)*NSIZEFB9(NCBUFF),
     +                      II+DI(KFRAME)*NSIZEFB9(NCBUFF),
     +                      NCBUFF+NMAXBUFF/2)
...
ccc                FPIXUSED(J,I)=0.
     ...
     +           SQRT(IMAGEN(J,I,NEWBUFF1+NMAXBUFF/2))/FPIXUSED(J,I)
            ...
            END DO
          END DO
C en aquellos pixels en los que no hemos sumado nada, introducimos un error 
C negativo
          DO I=1,NYMAXB9_
            DO J=1,NXMAXB9_
