C Rectifica la imagen actual y la almacena en el buffer NEWBUFF. En caso
C necesario, tambien rectifica la imagen de errores.
...
C los coeficientes AIJ,BIJ,AIJ_,BIJ_ y el numero de pixel en la imagen original.
...
C Nearest Neighbor
        IF(IRESAMPL.EQ.1)THEN
          DO I=I1,I2-1
            ...
            DO J=J1,J2-1
              ...
              IF(LERR)THEN
                IMAGEN(J-J1+1,I-I1+1,NEWBUFF+NMAXBUFF/2)=
     +           IMAGEN(JJ,II,NCBUFF+NMAXBUFF/2)
          ...
          END DO
C------------------------------------------------------------------------------
C Linear interpolation: aqui asumimos que los pixels distorsionados son,
C localmente, cuadrilateros (lados rectos). El error va a ser diminuto y de
C hecho tenemos mucha mas incertidumbre con el reparto de la sen~al en el
C pixel.
...
ccc               read(*,*)
              ...
              IF(LERR)THEN
                IMAGEN(J-J1+1,I-I1+1,NEWBUFF+NMAXBUFF/2)=
     +           SQRT(IMAGEN(J-J1+1,I-I1+1,NEWBUFF+NMAXBUFF/2))
              END IF
            END DO
            CALL SHOWPERC(I1,I2-1,1,I,NEXTINFO)
          END DO
...
C
C******************************************************************************
C Dibuja el poligono de 4 lados cuyos vertices se dan como parametros de
C entrada
...
C
C******************************************************************************
C Dibuja la interseccion del pixel problema sobre el pixel original, utilizando
C la informacion calculada por la subrutina SRCPAT. Esta subrutina devuelve
C el area de la interseccion.
...
ccc        INTEGER I,OLDCI
C------------------------------------------------------------------------------
ccc        CALL PGQCI(OLDCI)
ccc        CALL PGSCI(4)
ccc        CALL PGMOVE(XCOM(1,1),XCOM(2,1))
ccc        DO I=2,NCOR
ccc          CALL PGDRAW(XCOM(1,I),XCOM(2,I))
ccc        END DO
ccc        CALL PGDRAW(XCOM(1,1),XCOM(2,1))
ccc        CALL PGSCI(OLDCI)
C
C Nota: es importante tener valores cercanos a uno porque cuando el valor
C del pixel es grande, el valor de FAREA tiene un error muy grande (incluso
C da valores negativos). El modulo que llama a esta subrutina ya se encarga
C de pasar en XCOM valores cercanos a uno.
        FAREA=0.
        DO I1=1,NCOR
          IF(I1.EQ.NCOR)THEN
            I2=1
