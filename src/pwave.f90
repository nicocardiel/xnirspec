! Dibuja una escala con la calibracion en longitud de onda
...
! eje
          ...
          DO I=1,NXYMAX
            X0=REAL(J1)+REAL(I-1)/REAL(NXYMAX-1)*REAL(J2-J1)
...
! ticks grandes
          ...
          DO WHILE(IDUM.LT.NINT(XMAX*FACTOR))
            ...
            IF(IDUM.LT.NINT(XMAX*FACTOR))THEN
              ...
              IF((NINT(XP(1)).GE.NX1).AND.(NINT(XP(1)).LE.NX2).AND.(NINT(YP(1)).GE.NY1).AND.(NINT(YP(1)).LE.NY2))THEN
                CALL PGSCI(5)
                IF(NLINEA.EQ.1)THEN
...
!!!                  CALL FMAP(NMAP,AIJ_,BIJ_,X0,Y0+1.2*DY0,XP(3),YP(3))
!!!                  CALL PGPTXT(XP(3)+XCH/3.,YP(3),90.,0.0,CDUMMY(1:L))
                END IF
              END IF
              CALL PGSCI(1)
            END IF
