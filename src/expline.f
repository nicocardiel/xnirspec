C Usando una posicion de referencia, esta subrutina busca las lineas
C espectrales.
...
C------------------------------------------------------------------------------
        LDEBUGLOCAL=.TRUE.
C dibujamos linea en la direccion spectral
        IF(LDEBUGLOCAL)THEN
          XMIN=XMINBL(1)
          XMAX=XMAXBL(1)
          DO I=1,NXYMAX
          ...
          CALL PGSCI(1)
        END IF
C intersecciones de la linea local con los espectros del borde
     ...
     +   XC,XMAX,YMAX)
C dibujamos linea en la direccion espacial
        IF(LDEBUGLOCAL)THEN
          DO I=1,NXYMAX
        ...
        END IF
C------------------------------------------------------------------------------
C Grid en Y con NYFINDAL lineas: buscamos los picos en los NYFINDAL espectros
        DO NS=1,NYFINDAL
          ...
          END DO
          CALL FINDMAX(NAXIS(1,NCBUFF),1,NAXIS(1,NCBUFF),XP,ZP,
     +     NPEAKS,XPEAKS,ZPEAKS)
          ...
          END IF
          IF(LDEBUGLOCAL)THEN
            CALL SUBPLOT(NAXIS(1,NCBUFF),NX1,NX2,XP,ZP,XP,ZP,
     +       .TRUE.,.TRUE.,.FALSE.,.FALSE.,
     +       'wavelength direction','signal',' ',3,201,1.0)
            CALL SUBPLOTBIS(NPEAKS,1,NPEAKS,XPEAKS,ZPEAKS,XPEAKS,ZPEAKS,
     +       .FALSE.,.FALSE.,2,21,1.5)
          END IF
          DO I=1,NPEAKS
...
C------------------------------------------------------------------------------
C Reconocemos las lineas. Para ello, vamos recorriendo los NYFINDAL espectros,
C comprobando si un nuevo pico (no utilizado anteriormente) tiene nuevas
C identificaciones en el resto de los NYFINDAL-1 espectros.
C------------------------------------------------------------------------------
C Dado que exigimos tener al menos NINLINE picos detectados, no tenemos que
C hace el primer bucle que sigue a continuacion desde 1 hasta NYFINDAL, sino 
C que basta hacerlo desde 1 hasta NYFINDAL-NINLINE+1. Llegados al espectro
C numero NYFINDAL-NINLINE+2, quedan NINLINE-1 espectros por examinar, con lo
C cual nunca podremos obtener NINLINE picos.
C
...
c tomamos el siguiente pico no utilizado todavia
...
c calculamos extremos de la linea espectral
              CALL INTERSEC(NDEGBA00,COEFFBA00_,
     +         NDEGBL(1),COEFFBL(1,1),
     +         X0,XMIN,YMIN)
              CALL INTERSEC(NDEGBA00,COEFFBA00_,
     +         NDEGBL(NLINBL),COEFFBL(1,NLINBL),
...
C seguridad determinado por NMED)
              NMED=3
              LINSIDE=((XMIN.GT.REAL(1+NMED)).AND.
     +                 (XMIN.LT.REAL(NAXIS(1,NCBUFF)-NMED)).AND.
     +                 (YMIN.GT.REAL(1+NMED)).AND.
     +                 (YMIN.LT.REAL(NAXIS(2,NCBUFF)-NMED)).AND.
     +                 (XMAX.GT.REAL(1+NMED)).AND.
     +                 (XMAX.LT.REAL(NAXIS(1,NCBUFF)-NMED)).AND.
     +                 (YMAX.GT.REAL(1+NMED)).AND.
     +                 (YMAX.LT.REAL(NAXIS(2,NCBUFF)-NMED)))
C OK! estamos dentro
              IF(LINSIDE)THEN
c..............................................................................
C Predecimos posiciones esperadas de la linea en los demas NYFINDLA espectros,
C y comprobamos si hay algun pico cerca; la linea es aceptada si el numero
C de picos proximos es igual o mayor que NINLINE. Como estamos barriendo los
C espectros de abajo a arriba, no es necesario chequear los picos de los
C espectros que quedan por debajo del actual (esos picos ya han sido examinados
C y si no han sido ubicados en ninguna linea, tampoco deben ser utiles para
C los picos que se encuentran en el espectro actual y por encima). 
                ...
                DO NS_=NS+1,NYFINDAL
                  ...
                  END IF
                END DO
C si el numero de picos encontrados es suficientemente grande, ajustamos la
C linea y eliminamos los picos usados del pool de picos sin usar
                IF(NPEAKS.GE.NINLINE)THEN
                  NLINBA_=NLINBA_+1
                  CALL POLFIT(YF,XF,XF,NPEAKS,NDEGBA00+1,0,
     +             COEFFBA_(1,NLINBA_),CHISQR)
                  IF(LDEBUGLOCAL)THEN
                    CALL INTERSEC(NDEGBA00,COEFFBA_(1,NLINBA_),
     +               NDEGBL(1),COEFFBL(1,1),
     +               X0,XMIN,YMIN)
                    CALL INTERSEC(NDEGBA00,COEFFBA_(1,NLINBA_),
     +               NDEGBL(NLINBL),COEFFBL(1,NLINBL),
     +               X0,XMAX,YMAX)
                    ...
                    CALL SUBPLOT(NPEAKS,1,NPEAKS,YF,XF,XF,YF,
     +               .TRUE.,.TRUE.,.FALSE.,.FALSE.,
     +               'Y axis','X axis','Line #'//CDUMMY(1:L),2,17,1.5)
                    CALL SUBPLOTBIS(NXYMAX,1,NXYMAX,YP,XP,YP,XP,
     +               .FALSE.,.FALSE.,3,101,1.0)
                  END IF
        ...
        DO L=1,NLINBA_
          ...
          ELSE
            ...
            ELSE
              CCONT='n'
            END IF
            CCONT(1:1)=READC('Are you using this last fit (y/n/a=auto)',
     +       CCONT,'yna')
            IF(CCONT.EQ.'a')THEN
        ...
        IF(I.EQ.0) RETURN
C------------------------------------------------------------------------------
C si nos gusta el resultado, podemos actualizar las lineas BA que definen
C el boundary
        CUPDATE(1:1)=READC('Do you want to update BA lines (y/n)',
     +   'y','yn')
        IF(CUPDATE.EQ.'y')THEN
          CADD(1:1)=
     +     READC('[a]ppend to previous list or define [n]ew list',
     +     'a','an')
