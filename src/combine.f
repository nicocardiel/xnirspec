C Version 6-November-2000
C
C Nota: en la combinacion comprobamos que el fichero de errores no contiene
C sen~al negativa (la sen~al negativa en la imagen de errores se emplea, por 
C tanto, como mascara para indicar que pixels pueden sumarse; esto es esencial
C cuando sumamos imagenes que contienen huecos y queremos que el numero de
C cuentas se conserve adecuadamente).
...
C proteccion #2
        DO NF=1,NFRAMES_
          IF((NAXISFRAME(1,NF,NCBUFF).EQ.0).OR.
     +       (NAXISFRAME(2,NF,NCBUFF).EQ.0))THEN
            WRITE(*,*)
...
C numero de pixels umbral para deteccion de pixels calientes
     ...
     +     ' Times_Sigma for hot/cool pixels',CDUMMY)
        END IF
C------------------------------------------------------------------------------
C decidimos si pesamos la imagen suma con los errores
        CWEIGHT(1:1)=READC('Perform weighted sum (y/n)',CWEIGHT,'yn')
        IF(CWEIGHT.EQ.'y')THEN
          CWMODE(1:1)=READC('Weigth with [e]rrors or [s]ignal/noise',
     +     'e','es')
          WRITE(*,101) '* Note: data can be masked if error is below '//
     +     'a given threshold error'
...
C------------------------------------------------------------------------------
C------------------------------------------------------------------------------
C it is time to coadd the frames into a single image
...
C los frames iniciales se pueden usar en cada caso; aqui hay que tener cuidado
C porque, al contrario de lo que ocurre al combinar un box-9 formado por 
C imagenes de 256x256 pixels, en este caso tenemos que combinar varias imagenes
C que inicialmente pueden tener taman~os distintos!
        LANYCR=.FALSE.
        NEXTINFO=0
        DO I=1,NAXIS(2,NEWBUFF1)
          DO J=1,NAXIS(1,NEWBUFF1)
            ...
            DO K=1,NFRAMES_
              ...
              DO IJ=1,4 !recorremos los 4 posibles pixels
                ...
                END IF
                IF(INSIDE(JJ,1,NAXISFRAME(1,K,NCBUFF),
     +                    II,1,NAXISFRAME(2,K,NCBUFF)))THEN
                  IF(IMAGEN(JJ+DJ(K)*NSIZEFB9(NCBUFF),
     +             II+DI(K)*NSIZEFB9(NCBUFF),NCBUFF+NMAXBUFF/2).GE.
     +             0.0)THEN
                    KK=KK+1
                    PIXEL(KK)=IMAGEN(JJ+DJ(K)*NSIZEFB9(NCBUFF),
     +                               II+DI(K)*NSIZEFB9(NCBUFF),NCBUFF)
                    EPIXEL(KK)=
     +               IMAGEN(JJ+DJ(K)*NSIZEFB9(NCBUFF),
     +                      II+DI(K)*NSIZEFB9(NCBUFF),NCBUFF+NMAXBUFF/2)
...
C----->
              IF(KK.LE.4)THEN !si tenemos menos de cinco pixels
                DO NK=1,KK
                  ...
                  DO II=II_LAST(NK)-2,II_LAST(NK)+2
                    IF((II.GE.1).AND.
     +                 (II.LE.NAXISFRAME(2,K_LAST(NK),NCBUFF)))THEN
                      DO JJ=JJ_LAST(NK)-2,JJ_LAST(NK)+2
                        IF((JJ.GE.1).AND.
     +                    (JJ.LE.NAXISFRAME(1,K_LAST(NK),NCBUFF)))THEN
                          IF((II.NE.II_LAST(NK)).OR.
     +                       (JJ.NE.JJ_LAST(NK)))THEN
                            NPIXELSVECINOS=NPIXELSVECINOS+1
                            PIXELVECINO(NPIXELSVECINOS)=IMAGEN(
     +                       JJ+DJ(K_LAST(NK))*NSIZEFB9(NCBUFF),
     +                       II+DI(K_LAST(NK))*NSIZEFB9(NCBUFF),
     +                       NCBUFF)
                          END IF
                        END IF
                      END DO
     ...
     +               REAL(II_LAST(NK)+DI(K_LAST(NK))*NSIZEFB9(NCBUFF)),
     +               21)
                    CALL PGSCI(1)
                    LANYCR=.TRUE.
                  END IF
...
C eliminamos el pixel mas brillante si excede en TSHOT veces sigma
     ...
     +             REAL(II_LAST(KK)+DI(K_LAST(KK))*NSIZEFB9(NCBUFF)),
     +             21)
                  CALL PGSCI(1)
                  KK=KK-1
                  LANYCR=.TRUE.
...
C eliminamos el pixel mas debil si excede en -TSHOT veces sigma
     ...
     +             REAL(II_LAST(1)+DI(K_LAST(1))*NSIZEFB9(NCBUFF)),
     +             21)
                  CALL PGSCI(1)
                  DO NN=1,KK-1
                    PIXEL(NN)=PIXEL(NN+1)
...
ccc              IMAGEN(J,I,NEWBUFF1)=0.
ccc              IMAGEN(J,I,NEWBUFF1+NMAXBUFF/2)=0.
ccc              FPIXUSED(J,I)=0.
            ELSE
     ...
     +                   (EPIXEL(K)*EPIXEL(K))
              ...
              END DO
              DO K=1,KK
ccc                IMAGEN(J,I,NEWBUFF1)=IMAGEN(J,I,NEWBUFF1)+
ccc     +           PIXEL(K)*PIXELW(K)*PIXELF(K)
                IMAGEN(J,I,NEWBUFF1)=IMAGEN(J,I,NEWBUFF1)+
     +           PIXEL(K)*PIXELW(K)
ccc                IMAGEN(J,I,NEWBUFF1+NMAXBUFF/2)=
ccc     +           IMAGEN(J,I,NEWBUFF1+NMAXBUFF/2)+
ccc     +           EPIXEL(K)*EPIXEL(K)*PIXELW(K)*PIXELW(K)*
ccc     +           PIXELF(K)*PIXELF(K)
                IMAGEN(J,I,NEWBUFF1+NMAXBUFF/2)=
     +           IMAGEN(J,I,NEWBUFF1+NMAXBUFF/2)+
     +           EPIXEL(K)*EPIXEL(K)*PIXELW(K)*PIXELW(K)
ccc                FPIXUSED(J,I)=FPIXUSED(J,I)+PIXELF(K)
              END DO
              IMAGEN(J,I,NEWBUFF1)=IMAGEN(J,I,NEWBUFF1)/FPIXUSED(J,I)
              IMAGEN(J,I,NEWBUFF1+NMAXBUFF/2)=
     +         SQRT(IMAGEN(J,I,NEWBUFF1+NMAXBUFF/2))/FPIXUSED(J,I)
            END IF
          END DO
          CALL SHOWPERC(1,NAXIS(2,NEWBUFF1),1,I,NEXTINFO)
        END DO
C..............................................................................
C salvamos en otro buffer el numero de pixels utilizados para el calculo de 
C cada pixel (ademas, en aquellos pixels en los que no hemos sumado nada, 
C introducimos un error negativo)
...
C------------------------------------------------------------------------------
C refinamos las dimensiones iniciales y calculamos unas dimensiones mas
C ajustadas al taman~o real de la imagen combinada (notar que el limite en la
C busqueda lo establezco a -0.5, cuando sabemos que los errores se han
C establecido en -1.0 para indicar pixels no utilizados; de esta forma
C garantizamos que no cometemos ningun error de redondeo al comparar numeros
C reales)
        NREMOV_XMINB9=0
        J=0
        LMORE=.TRUE.
...
C
        ...
        END DO
C
        WRITE(*,100) '> No. pixels to remove in Xmin, Xmax, '//
     +   'Ymin, Ymax: '
        WRITE(*,*) NREMOV_XMINB9,NREMOV_XMAXB9,NREMOV_YMINB9,
     +   NREMOV_YMAXB9
        ...
        WRITE(*,101) CDUMMY(1:L)
C..............................................................................
C movemos imagenes y recortamos al taman~o final
        IF((NREMOV_XMINB9.GT.0).OR.(NREMOV_YMINB9.GT.0))THEN
          DO I=1,NYMAXB9_EFF
            DO J=1,NXMAXB9_EFF
              IMAGEN(J,I,NEWBUFF1)=
     +         IMAGEN(J+NREMOV_XMINB9,I+NREMOV_YMINB9,NEWBUFF1)
              IMAGEN(J,I,NEWBUFF1+NMAXBUFF/2)=
     +         IMAGEN(J+NREMOV_XMINB9,I+NREMOV_YMINB9,
     +         NEWBUFF1+NMAXBUFF/2)
              IMAGEN(J,I,NEWBUFF2)=
     +         IMAGEN(J+NREMOV_XMINB9,I+NREMOV_YMINB9,NEWBUFF2)
            END DO
          END DO
        END IF
...
C------------------------------------------------------------------------------
        ...
        CALL STATISTIC(NEWBUFF1,NX1,NX2,NY1,NY2,.FALSE.,.TRUE.,.TRUE.,
     +   0.0,.FALSE.)
