...
!------------------------------------------------------------------------------
        IF(COPC.EQ.'x')THEN
          RETURN
!------------------------------------------------------------------------------
! define vertical pattern of BA lines
        ELSEIF(COPC.EQ.'a')THEN
          ...
          DO L=1,NLINBA
            NDEGBA(L)=NDEGBA00
            COEFFBA(1,L)=XMINBL(1)+REAL(L)*(XMAXBL(1)-XMINBL(1))/REAL(NLINBA+1)
...
! load boundary from file
        ELSEIF(COPC.EQ.'l')THEN
          ...
          DO WHILE(.NOT.LOGFILE)
            FILENAME=READC('Boundary file name (none=EXIT)','*.boun','@')
...
! delete BL line with mouse
        ELSEIF(COPC.EQ.'r')THEN
          ...
          IF(CH.EQ.'A')THEN
            ...
            ELSE
              DO I=1,NXYMAX
                XP(I)=XMINBL(LMIN)+REAL(I-1)/REAL(NXYMAX-1)*(XMAXBL(LMIN)-XMINBL(LMIN))
...
! delete BA line with mouse
        ELSEIF(COPC.EQ.'d')THEN
          ...
          IF(CH.EQ.'A')THEN
            ...
            ELSE
              DO I=1,NXYMAX
                YP(I)=YMINBA(LMIN)+REAL(I-1)/REAL(NXYMAX-1)*(YMAXBA(LMIN)-YMINBA(LMIN))
          ...
          END IF
!------------------------------------------------------------------------------
! test shrinking/stretching in BL
        ELSEIF(COPC.EQ.'1')THEN
          NREF=READILIM('Reference BL line','1',1,NLINBL)
          WRITE(CDUMX,*) NREF
          L1=TRUEBEG(CDUMX)
          L2=TRUELEN(CDUMX)
! XF va a ser la escala de referencia
! YF va a ser la escala que vamos a ajustar frente a XF
          CALL INTERSEC(NDEGBA(1),COEFFBA(1,1),NDEGBL(NREF),COEFFBL(1,NREF),0.5*(XMINBA(1)+XMAXBA(1)),X1,Y1)
          XF(1)=0.
          DO L=2,NLINBA
            CALL INTERSEC(NDEGBA(L),COEFFBA(1,L),NDEGBL(NREF),COEFFBL(1,NREF),0.5*(XMINBA(L)+XMAXBA(L)),X2,Y2)
            XF(L)=ARCLENGTH(NDEGBL(NREF),COEFFBL(1,NREF),X1,X2,100)
          END DO
          DO LL=1,NLINBL
            IF(LL.NE.NREF)THEN !si no es la escala de referencia la ajustamos
              ...
              LL1=TRUEBEG(CDUMY)
              LL2=TRUELEN(CDUMY)
              CALL INTERSEC(NDEGBA(1),COEFFBA(1,1),NDEGBL(LL),COEFFBL(1,LL),0.5*(XMINBA(1)+XMAXBA(1)),X1,Y1)
              YF(1)=0.
              DO L=2,NLINBA
                CALL INTERSEC(NDEGBA(L),COEFFBA(1,L),NDEGBL(LL),COEFFBL(1,LL),0.5*(XMINBA(L)+XMAXBA(L)),X2,Y2)
                YF(L)=ARCLENGTH(NDEGBL(LL),COEFFBL(1,LL),X1,X2,100)-XF(L)
              END DO
          ...
          END DO
!------------------------------------------------------------------------------
! test shrinking/stretching in BA
...
! XF va a ser la escala de referencia
! YF va a ser la escala que vamos a ajustar frente a XF
          CALL INTERSEC(NDEGBA(NREF),COEFFBA(1,NREF),NDEGBL(1),COEFFBL(1,1),0.5*(XMINBA(NREF)+XMAXBA(NREF)),X1,Y1)
          XF(1)=0.
          DO L=2,NLINBL
            CALL INTERSEC(NDEGBA(NREF),COEFFBA(1,NREF),NDEGBL(L),COEFFBL(1,L),0.5*(XMINBA(NREF)+XMAXBA(NREF)),X2,Y2)
            XF(L)=ARCLENGTH(NDEGBA(NREF),COEFFBA(1,NREF),Y1,Y2,100)
          END DO
          DO LL=1,NLINBA
            IF(LL.NE.NREF)THEN !si no es la escala de referencia la ajustamos
              WRITE(CDUMY,*) LL
              LL1=TRUEBEG(CDUMY)
              LL2=TRUELEN(CDUMY)
              CALL INTERSEC(NDEGBA(LL),COEFFBA(1,LL),NDEGBL(1),COEFFBL(1,1),0.5*(XMINBA(LL)+XMAXBA(LL)),X1,Y1)
              YF(1)=0.
              DO L=2,NLINBL
                CALL INTERSEC(NDEGBA(LL),COEFFBA(1,LL),NDEGBL(L),COEFFBL(1,L),0.5*(XMINBA(LL)+XMAXBA(LL)),X2,Y2)
                YF(L)=ARCLENGTH(NDEGBA(LL),COEFFBA(1,LL),Y1,Y2,100)-XF(L)
              END DO
