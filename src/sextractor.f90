...
8           CLOSE(44)
          ...
          IF(LOGCATFILE)THEN
            ...
            END IF
            IF(L_XMIN_IMAGE.AND.L_XMAX_IMAGE.AND.L_YMIN_IMAGE.AND.L_YMAX_IMAGE)THEN
              WRITE(*,101) '(2) Plot rectangle which encloses the detected object'
          ...
          ELSEIF(COPC.EQ.'R')THEN
            ...
            IF(CREF.EQ.'y')THEN
              ...
              DO WHILE(.NOT.LOGFILE)
                REFFILE=READC('Reference image to be used for detection of sources','*.fits','@')
                IF((INDEX(REFFILE,'*').NE.0).OR.(INDEX(REFFILE,'?').NE.0))THEN
                  ISYSTEM=SYSTEMFUNCTION('ls '//REFFILE(1:TRUELEN(REFFILE)))
                ELSE
            ...
            IF(LOK)THEN
              ...
              L1=TRUEBEG(SEXPATH)
              L2=TRUELEN(SEXPATH)
              IF(L_PHOTFLAM(NCBUFF).AND.L_PHOTZPT(NCBUFF).AND.L_EXPTIME(NCBUFF))THEN  !HST flux calibration keywords available
                ...
                ZEROPOINT(NCBUFF)=PHOTZPT(NCBUFF)-2.5*ALOG10(PHOTFLAM(NCBUFF)/EXPTIME(NCBUFF))
              ...
              IF(ZEROPOINT(NCBUFF).NE.0.0)THEN
                ...
                IF(CREF.EQ.'y')THEN
                  ...
                  ISYSTEM=SYSTEMFUNCTION(SEXPATH_(L1:L2)//'/sex '//REFFILE(LLL1:LLL2)//',xyzxyzxyz.fits '// &
                   '-MAG_ZEROPOINT '//CDUMMY(LL1:LL2))
                ELSE
                  WRITE(*,101) '% '//SEXPATH_(L1:L2)//'/sex \\'
                  WRITE(*,100) '  xyzxyzxyz.fits -MAG_ZEROPOINT '
                  WRITE(*,101) CDUMMY(LL1:LL2)
                  ISYSTEM=SYSTEMFUNCTION(SEXPATH_(L1:L2)//'/sex '//'xyzxyzxyz.fits -MAG_ZEROPOINT '//CDUMMY(LL1:LL2))
                END IF
              ELSE
                IF(CREF.EQ.'y')THEN
                  WRITE(*,101) '% '//SEXPATH_(L1:L2)//'/sex \\'
                ...
                ELSE
                  ...
                  WRITE(*,101) 'xyzxyzxyz.fits'
                  ISYSTEM=SYSTEMFUNCTION(SEXPATH_(L1:L2)//'/sex '//'xyzxyzxyz.fits')
                END IF
              END IF
              ISYSTEM=SYSTEMFUNCTION('rm -f xyzxyzxyz.fits')
...
20          READ(44,101,END=22) CLINEA
            IF(CLINEA(1:1).EQ.'#')THEN
              ...
              IF(CLINEA(7:22).EQ.'X_IMAGE') READ(CLINEA(2:5),*) N_X_IMAGE
              IF(CLINEA(7:22).EQ.'Y_IMAGE') READ(CLINEA(2:5),*) N_Y_IMAGE
              IF(CLINEA(7:22).EQ.'XMIN_IMAGE') READ(CLINEA(2:5),*) N_XMIN_IMAGE
              IF(CLINEA(7:22).EQ.'XMAX_IMAGE') READ(CLINEA(2:5),*) N_XMAX_IMAGE
              IF(CLINEA(7:22).EQ.'YMIN_IMAGE') READ(CLINEA(2:5),*) N_YMIN_IMAGE
              IF(CLINEA(7:22).EQ.'YMAX_IMAGE') READ(CLINEA(2:5),*) N_YMAX_IMAGE
              IF(CLINEA(7:22).EQ.'A_IMAGE') READ(CLINEA(2:5),*) N_A_IMAGE
              IF(CLINEA(7:22).EQ.'B_IMAGE') READ(CLINEA(2:5),*) N_B_IMAGE
              IF(CLINEA(7:22).EQ.'THETA_IMAGE') READ(CLINEA(2:5),*) N_THETA_IMAGE
              IF(CLINEA(7:22).EQ.'CLASS_STAR') READ(CLINEA(2:5),*) N_CLASS_STAR
            ELSE
              GOTO 22
            END IF
            GOTO 20
...
24          READ(44,101,END=26) CLINEA
            IF(COPC.EQ.'i')THEN !info
              ...
              IF(N0.EQ.N0_)THEN
                ...
                CALL PGSCI(6)
                IF((X_IMAGE.GT.XMIN).AND.(X_IMAGE.LT.XMAX).AND.(YMAX_IMAGE.GT.YMIN).AND.(YMAX_IMAGE.LT.YMAX))THEN
                  CALL PGSLW(4)
            ...
            ELSEIF(COPC.EQ.'3')THEN !isophotal limit
              ...
              DO I=0,360
                ...
                R0=A_IMAGE*B_IMAGE/SQRT(A_IMAGE*A_IMAGE-(A_IMAGE*A_IMAGE-B_IMAGE*B_IMAGE)*COS(THETA0)*COS(THETA0))
                X0=R0*COS(THETA0)
                Y0=R0*SIN(THETA0)
                XELL(I+1)=X_IMAGE+X0*COS(THETA_IMAGE)+Y0*SIN(THETA_IMAGE)
                YELL(I+1)=Y_IMAGE-X0*SIN(THETA_IMAGE)+Y0*COS(THETA_IMAGE)
              END DO
              CLASS_STAR=LEECOLUMN(CLINEA,N_CLASS_STAR)
              IF(CLASS_STAR.GT.0.8) CALL PGSLS(4)
              CALL PGSCI(5)
