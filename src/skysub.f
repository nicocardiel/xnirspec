C resta el cielo en una imagen con distorsion
...
ccc        REAL XCUT_ERR(NXMAX*NOVERSAMPMAX)
...
C------------------------------------------------------------------------------
        ...
        END IF
C------------------------------------------------------------------------------
        LERR=(NCBUFF.LE.NMAXBUFF/2).AND.(NEWBUFF1.LE.NMAXBUFF/2).AND.
     +       (NEWBUFF2.LE.NMAXBUFF/2)
C------------------------------------------------------------------------------
...
C------------------------------------------------------------------------------
C IFSCANSKY indica que espectros de la imagen corregida vamos a utilizar para
C calcular el cielo
        DO I=I1,I2-1
...
C
        ...
        WRITE(*,*) NSCANSKY
C------------------------------------------------------------------------------
C IFSCANOBJ indica que espectros de la imagen contienen el/los objeto/s de
C interes
        DO I=I1,I2-1
...
C------------------------------------------------------------------------------
C------------------------------------------------------------------------------
C Extraemos el espectro del cielo, usando sobremuestreado si se solicita.
...
ccc               FAREATOT=1.0 !lo sabemos, por lo que no hacemos PPOLYN
...
C------------------------------------------------------------------------------
C La coordenada X de los nuevos subpixels se refieren al centro de dichos
C subpixels. Por esa razón añadimos el factor 0.5*FOVERSAMP.
        DO J=1,(J2-J1)*NOVERSAMP
          XP(J)=REAL(J1)+REAL(J-1)*FOVERSAMP+0.5*FOVERSAMP
          write(77,*) xp(j),xcut(j)
        END DO
        CALL SUBPLOT((J2-J1)*NOVERSAMP,1,(J2-J1)*NOVERSAMP,
     +   XP,XCUT,XP,XCUT,
     +   .TRUE.,.TRUE.,.FALSE.,.FALSE.,
     +   'wavelength direction','signal','averaged sky spectrum',
     +   3,201,1.0)
C------------------------------------------------------------------------------
C Creamos una imagen distorsionada usando el espectro de cielo promedio
C anterior.
            ...
            IF(IFSCANSKY(ITEST))THEN
              ...
              DO II=IYMIN,IYMAX
                DO JJ=IXMIN,IXMAX
                  ...
                  CALL SRCPAT(4,XCOR1,4,XCOR2,NCOR,XCOM)
                  CALL PPOLYN(NCOR,XCOM,FAREA)
                  IMAGEN(JJ,II,NEWBUFF1)=IMAGEN(JJ,II,NEWBUFF1)+
     +             XCUT(J)*FAREA/FAREASUBPIXEL
...
!                  END IF
        ...
        END DO
C------------------------------------------------------------------------------
C Extraemos de nuevo un espectro de cielo, pero esta vez usamos el espectro
C inicial para repartir la señal dentro de cada píxel.
        WRITE(*,100) 'Iteration #'
        WRITE(*,*) NITER
...
!     +                 FAREA/FAREASUBPIXEL*XCUT(J)/IMAGEN(JJ,II,NCBUFF)
                      ...
                      YCUT(K)=YCUT(K)+XCUT(J)*FAREA/FAREASUBPIXEL+
     +                 (IMAGEN(JJ,II,NCBUFF)*IMAGEN(JJ,II,NEWBUFF2)-
...
C
     ...
     +         SQRT(IMAGEN(J,I,NEWBUFF1+NMAXBUFF/2))
            END DO
          END DO
        END IF
C------------------------------------------------------------------------------
C restamos por fin la imagen
