...
!omment
!
! SUBROUTINE LUDCMPD(A,N,NDIM,ORDER,SCALEROW,IOK,IPAR)
!
! Input: A,N,NDIM
...
!
! DOUBLE PRECISION A(NDIM,NDIM) -> matrix of coefficients
...
! DOUBLE PRECISION SCALEROW(N) -> vector holding scaling factors applied 
!                to each row
...
!!!10      CALL PIVOTED(A,N,NDIM,ORDER,1,IPAR)
        CALL PIVOTED(A,N,NDIM,ORDER,1,IPAR)
!------------------------------------------------------------------------------
! si el pivote es muy pequen~o, podemos tener una matriz singular o casi 
! singular
        CALL LITTLEPIVOTED(A(1,1),LOK)
        IF(.NOT.LOK)THEN
          IOK=1
          RETURN
        END IF
...
! en caso necesario podemos intercambiar filas
          CALL PIVOTED(A,N,NDIM,ORDER,J,IPAR)
! comprobamos si el pivote es demasiado pequen~o
          CALL LITTLEPIVOTED(A(J,J),LOK)
          IF(.NOT.LOK)THEN
            IOK=J
            RETURN
          END IF
...
! dependiendo del numero de cambios realizados en las filas.
        SUBROUTINE PIVOTED(A,N,NDIM,ORDER,J0,IPAR)
        IMPLICIT NONE
!
        INTEGER N,NDIM
        DOUBLE PRECISION A(NDIM,NDIM)
...
! una matriz problematica.
        SUBROUTINE LITTLEPIVOTED(A,LOK)
        IMPLICIT NONE
!
        DOUBLE PRECISION A
        LOGICAL LOK
