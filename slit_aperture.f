C Version 06-February-2001
C Determina la fraccion de luz que atraviesa una rendija con una anchura y
C orientacion determinadas. Tambien realiza simulaciones de Monte Carlo para
C estimar el error.
...
C Proteccion
        ...
        END IF
C------------------------------------------------------------------------------
C Definimos los pixels que definen el objeto que queremos medir
        WRITE(*,100) 'Use the mouse to indicate the image regions'
        WRITE(*,101) 'that define the object'
C inicializamos la mascara
        DO I=NY1,NY2
          DO J=NX1,NX2
            MASKPIXEL(J-NX1+1,I-NY1+1)=.FALSE.
          END DO
        END DO
C definimos regiones a utilizar
        LOOP=.TRUE.
        DO WHILE(LOOP)
...
C Dibujamos la rendija
        ...
        CALL PGSCI(1)
C------------------------------------------------------------------------------
C Para el calculo de las intersecciones, cambiamos el origen de la escala
C para no tener valores demasiado grandes
...
C------------------------------------------------------------------------------
C Calculamos el area dentro de la rendija y el area total del objeto
C seleccionado.
C NOTA: el procedimiento calcula incluso las fracciones de pixel dentro de la
C rendija, por lo que funciona en un caso general, cualesquiera que sean las
C dimensiones relativas de los pixels y de la rendija considerada.
        SUM_TOTAL=0.0
        SUM_INSLIT=0.0
        DO I=NY1,NY2
          DO J=NX1,NX2
     ...
     +                    XCOR2(1,4)+XC_SLIT,XCOR2(2,4)+YC_SLIT,8)
        ...
        WRITE(*,*) SUM_INSLIT/SUM_TOTAL
C------------------------------------------------------------------------------
C Simulaciones para estimar errores
        NSIMUL=READILIM('No. of simulations to estimate errors',
     +   '100',0,NSIMULMAX)
        IF(NSIMUL.GT.0)THEN
          ...
          DO K=1,NSIMUL
            ...
            ERRORX=1.41421356*ERR_XY_SLIT*
     +       SQRT(-1.*LOG(1.-R1))*COS(2.*PI*R2)
            R1=RANDOMNUMBER(NSEED)
            R2=RANDOMNUMBER(NSEED)
            ERRORY=1.41421356*ERR_XY_SLIT*
     +       SQRT(-1.*LOG(1.-R1))*COS(2.*PI*R2)
            R1=RANDOMNUMBER(NSEED)
            R2=RANDOMNUMBER(NSEED)
            ERRORPA=1.41421356*ERR_PA_SLIT*
     +       SQRT(-1.*LOG(1.-R1))*COS(2.*PI*R2)
            ...
            DO I=NY1,NY2
              DO J=NX1,NX2
                IF(MASKPIXEL(J-NX1+1,I-NY1+1))THEN
                  ...
                  IF(NCOR.GT.0)THEN !hay interseccion
                    CALL PPOLYN(NCOR,XCOM,FAREA)
                    SUM_INSLIT_(K)=SUM_INSLIT_(K)+
     +               FAREA*IMAGEN(J,I,NCBUFF)
                  END IF
                END IF
              END DO
            END DO
          ...
          WRITE(*,*) SUM_INSLIT_MEAN/SUM_TOTAL,' +/-',
     +     SUM_INSLIT_SIGMA/SUM_TOTAL
        END IF
C------------------------------------------------------------------------------
100     FORMAT(A,$)
101     FORMAT(A)
