C Desplaza la imagen NBUFF2 para que coincida con la imagen NBUFF1 usando
C el offset que se mide al usar un mismo objeto en las dos imágenes
C
...
C------------------------------------------------------------------------------
        ...
        WRITE(*,101) '============================================'
C pedimos buffer de referencia
        NBUFF1=NCBUFF      !por defecto suponemos que va a ser el buffer activo
        LOOP=.TRUE.
        DO WHILE(LOOP)
          WRITE(CDUMMY,*) NBUFF1
          NBUFF1=READILIM('Buffer # of REFERENCE image (0=exit)',
     +     CDUMMY,0,NMAXBUFF/2)
          ...
          END IF
        END DO
C pedimos buffer con imagen a ser desplazada
        LOOP=.TRUE.
        DO WHILE(LOOP)
...
C------------------------------------------------------------------------------ 
C desplazamos la imagen (el codigo aqui lo copio de la subrutina SUBIMATH,
C aunque no manejamos imágenes de errores y la imagen de destino se
C sobreescribe; es decir, es una version simplificada)
        J0=INT(XOFFSET)
        FJ0=XOFFSET-REAL(J0)
        I0=INT(YOFFSET)
        FI0=YOFFSET-REAL(I0)
          ...
          DO J=1,NAXIS1
            ...
            IF((J1.GE.1).AND.(J1.LE.NAXIS1))THEN
              IF((I1.GE.1).AND.(I1.LE.NAXIS2))THEN
                IMAGEN_(J1,I1)=IMAGEN_(J1,I1)+
     +           IMAGEN(J,I,NBUFF2)*(1.-ABS(FI0))*(1.-ABS(FJ0))
              END IF
              IF((I2.GE.1).AND.(I2.LE.NAXIS2))THEN
                IMAGEN_(J1,I2)=IMAGEN_(J1,I2)+
     +           IMAGEN(J,I,NBUFF2)*ABS(FI0)*(1.-ABS(FJ0))
              END IF
            END IF
            IF((J2.GE.1).AND.(J2.LE.NAXIS1))THEN
              IF((I1.GE.1).AND.(I1.LE.NAXIS2))THEN
                IMAGEN_(J2,I1)=IMAGEN_(J2,I1)+
     +           IMAGEN(J,I,NBUFF2)*(1.-ABS(FI0))*ABS(FJ0)
              END IF
              IF((I2.GE.1).AND.(I2.LE.NAXIS2))THEN
                IMAGEN_(J2,I2)=IMAGEN_(J2,I2)+
     +           IMAGEN(J,I,NBUFF2)*ABS(FI0)*ABS(FJ0)
              END IF
            END IF
          END DO
        END DO
C introducimos en el buffer de destino el resultado
        DO I=1,NAXIS2
          DO J=1,NAXIS1
            IMAGEN(J,I,NBUFF2)=IMAGEN_(J,I)
          END DO
        END DO
C dibujamos la segunda imagen en su nueva posición
        CALL SUBLOOK(.TRUE.,NBUFF2,.FALSE.)
C------------------------------------------------------------------------------ 
100     FORMAT(A,$)
101     FORMAT(A)
